...
public class AdminPerson extends OC_Object{
    ...
    public Element toXmlElement(String destinationPersonId){
        ...
        patient.addAttribute("mpiid",ScreenHelper.checkString(adminextends.get("mpiid")));
    ...
    public void fromXmlElement(Element patient,boolean bIncludePersonId){
		...
		if(SH.c(patient.attributeValue("mpiid")).length()>0) {
			adminextends.put("mpiid",patient.attributeValue("mpiid"));
		}
    ...
    public boolean saveToDB(Connection connection) {
        ...
        try {
            ...
            if(SH.c(this.sourceid).length()==0) {
            	this.sourceid="0";
            }
            if(SH.c(sSourceID).length()==0) {
            	sSourceID="0";
            }
            ...
            Debug.println("1: bReturn = "+bReturn);
            ...
            if ((bReturn)&&(sPersonID.trim().length()==0)) {
                ...
                if ((sPersonID.trim().length()>0)&&(this.sourceid.trim().length()>0)) {
                    ...
                    if(!ScreenHelper.executeQuery(ps)) {
                        ...
                        if(!ScreenHelper.executeQuery(ps)) {
                            SH.syslog("B.2");
                            SH.syslog("personid="+sPersonID);
                            SH.syslog("natreg="+ScreenHelper.checkString(getID("natreg")));
                            SH.syslog("lastname="+this.lastname);
                            SH.syslog("sourceid="+sSourceID);
                        	bReturn=false;
                        }
                    }
                    if (ps!=null) ps.close();
    ...
    public boolean sendMPIRegistrationMessage() {
    	...
    	try {
    		return be.openclinic.system.Gmail.sendMessage(SH.cs("mpi.gmail.username","openclinic.mailrobot@ict4d.be"), SH.cs("mpi.gmail.password","nopass"), SH.cs("mpi.from", "openclinic.mailrobot@ict4d.be"), SH.cs("mpi.fromName","KHIN Network"), sTo, sFrom, sSubject, sMessage, SH.cs("mpi.logo","/var/tomcat/webapps/openclinic/_img/icons/khinfavicon-32x32.png"));
    	} catch (Exception e) {
    ...
    private static String getChildServices(String sParentServiceID) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if(!sReturnServiceIDs.contains("'"+sTmpServiceID+"'")) {
