...
public class DHIS2Helper {
	...
	public static void sendToServer(DataValueSet dataValueSet,Element dataset){
		sendToServer(dataValueSet,dataset,null);
	}
	...
	public static void sendToServer(DataValueSet dataValueSet,Element dataset, JspWriter jspWriter){
		String dataSetName = dataset.attributeValue("label");
		String DHIS2_SERVER_URI = MedwanQuery.getInstance().getConfigString(SH.c(dataset.attributeValue("dhis2serverprefix",""))+"dhis2_server_uri","https://dhis.snis.bi");
		String DHIS2_SERVER_BASE_API = MedwanQuery.getInstance().getConfigString(SH.c(dataset.attributeValue("dhis2serverprefix",""))+"dhis2_server_api","/api");
		String DHIS2_SERVER_PORT = MedwanQuery.getInstance().getConfigString(SH.c(dataset.attributeValue("dhis2serverprefix",""))+"dhis2_server_port","443");
		String OC_DHIS2_USER_NAME = MedwanQuery.getInstance().getConfigString(SH.c(dataset.attributeValue("dhis2serverprefix",""))+"dhis2_server_username","fverbeke");
		String OC_DHIS2_USER_PWD = MedwanQuery.getInstance().getConfigString(SH.c(dataset.attributeValue("dhis2serverprefix",""))+"dhis2_server_pwd","nopass");
        DHIS2Server server = new DHIS2Server(DHIS2_SERVER_URI, DHIS2_SERVER_BASE_API, DHIS2_SERVER_PORT);
		try {
			if(jspWriter!=null){
				jspWriter.print("Transmitting <b>"+dataValueSet.toXMLString().length()+" bytes</b> for data set <b>["+dataSetName+"]</b><li>to <font style='color: blue'>"+DHIS2_SERVER_URI+"</font>...</li> <script>document.body.scrollTop = document.body.scrollHeight;</script>");
				jspWriter.flush();
			}
        ...
        try {
            ...
            try {
            	...
            	System.out.println("This is a JSON response! Converting it to XML");
            	postResponse = "<root>"+XML.toString(json)+"</root>";
                System.out.println("Transformed POST response = "+postResponse);
            }
    		...
    		try {
    			if(jspWriter!=null){
    				String conflict="";
    				Element eConflicts=root.element("conflicts");
    				if(eConflicts!=null) {
	    				Iterator<Element> iConflicts=eConflicts.elementIterator("conflict");
	    				while(iConflicts.hasNext()) {
	    					Element eConflict = iConflicts.next();
	    					if(conflict.length()>0) {
	    						conflict+=", ";
	    					}
	    					conflict+=eConflict.attributeValue("value");
	    				}
		    			jspWriter.print("<li>Status: <b>"+root.elementText("status")+" </b></li>"+(conflict.length()>0?"<li><font style='color: red;font-weight: bolder'>"+conflict+"</font></li>":"")+"<br/>");
		    			jspWriter.flush();
    				}
    			}
    		} catch (Exception io) {
			...
			try {
				...
				if(!activeDataSet.equals(title) || !activeAttribute.equalsIgnoreCase(attribute)){
					...
					if(!activeDataSet.startsWith(getDataSetTitle(root.attributeValue("dataSet"),dataSetLabel))){
						...
						html.append("<table width='100%'><tr><td style='height: 20px;background-color: "+getDataSetColor(root.attributeValue("dataSet"),dataSetLabel)+";color:white;font-weight: bolder' colspan='"+(1+columns)+"'>"+getDataSetTitle(root.attributeValue("dataSet"),dataSetLabel).toUpperCase()+"</td></tr>");
						bColumnsDrawn=false;
					}
	...
	public static String getDataSetColor(String uid,String dataSetLabel){
		String dhis2document=MedwanQuery.getInstance().getConfigString("templateDirectory","/var/tomcat/webapps/openclinic/_common_xml")+"/"+MedwanQuery.getInstance().getConfigString("dhis2document","dhis2.bi.xml");
        SAXReader reader = new SAXReader(false);
        Document document;
		try {
			...
			while(i.hasNext()){
				Element dataset = (Element)i.next();
				if((activeDataSet.length()==0 || !activeDataSet.startsWith(dataSetLabel.split("\\|")[0]) || dataset.attributeValue("notitle")==null) && dataset.attributeValue("uid").equals(uid) && dataset.attributeValue("label").startsWith(dataSetLabel)){
					return ScreenHelper.checkString(dataset.attributeValue("color"),"#575757");
				}
			}
		} catch (DocumentException e) {
			e.printStackTrace();
