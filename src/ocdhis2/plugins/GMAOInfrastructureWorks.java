	...
	public Vector getResults(Date begin, Date end, Element dataset, Hashtable pluginParameters) {
		...
		try {
			...
			if(rootService.length()>0) {
				...
				double[][] results = new double[categories.length][8];
					...
					for(int i=0;i<categories.length;i++) {
						if(score.startsWith(categories[i]+";")) {
							...
							if(type.equalsIgnoreCase("2")) { //new construction
								...
								results[i][4]+=number;
								if(currency.equalsIgnoreCase("bif")) results[i][5]+=amount;
								if(currency.equalsIgnoreCase("usd")) results[i][6]+=amount;
								if(currency.equalsIgnoreCase("eur")) results[i][7]+=amount;
							}
							break;
						}
					}
