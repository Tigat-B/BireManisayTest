...
public class CreateTransactionAction extends Action {
    ...
    public ActionForward perform(ActionMapping mapping,
        ...
        try {
            ...
            Debug.println("Create transaction "+transactionType);
            //Check existence of CONTEXT_ENCOUNTERUID here!!!!!!!!!!!!
            Connection conn = SH.getOpenClinicConnection();
            PreparedStatement ps = conn.prepareStatement("select * from transactionitems where transactiontypeid=? and itemtypeid=?");
            ps.setString(1, transactionType);
            ps.setString(2, "be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CONTEXT_ENCOUNTERUID");
            ResultSet rs = ps.executeQuery();
            if(!rs.next()) {
            	rs.close();
            	ps.close();
            	ps = conn.prepareStatement("insert into transactionitems(transactiontypeid,itemtypeid,defaultvalue,modifier,priority,updatetime) values(?,?,'','',null,?)");
            	ps.setString(1, transactionType);
            	ps.setString(2, "be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CONTEXT_ENCOUNTERUID");
            ...
            if (transactionType.startsWith(be.mxs.common.model.vo.healthrecord.IConstants.TRANSACTION_TYPE_VACCINATION)) {
                ...
                Debug.println("actionForward="+MedwanQuery.getInstance().getForward(transactionType)+"&ts=" + new java.util.Date().hashCode()+extraParameters);
            }
        }
        catch (Exception e) {
            try {
