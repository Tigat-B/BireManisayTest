...
public class HealthRecordVO implements Serializable, IIdentifiable {
    ...
    public static HealthRecordVO getFromPersonId(String personid) {
    	return MedwanQuery.getInstance().getMedicalRecord(personid);
    }

    public static HealthRecordVO get(String healthRecordId) {
    	return MedwanQuery.getInstance().getMedicalRecord(""+MedwanQuery.getInstance().getPersonIdFromHealthrecordId(Integer.parseInt(healthRecordId)));
    }

    public static Vector<TransactionVO> getTransactions(int healthrecordid){
