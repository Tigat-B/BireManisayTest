...
public class MedwanQuery {
    ...
    public void getSubServices(String serviceid, Vector services){
        ...
        try{
            ...
            ResultSet rs = ps.executeQuery();
            while (rs.next()){
            	String s = rs.getString("serviceid");
            	if(!services.contains(s)) {
            		getSubServices(s, services);
            	}
            }
            rs.close();
            ps.close();
    ...
    //--- STORE LABEL -----------------------------------------------------------------------------
    public void storeLabelWithDelete(String labelType, String labelId, String labelLang, String labelValue, int updateUserId){
        PreparedStatement ps = null;
        Connection oc_conn=getOpenclinicConnection();
        try{
    ...
    public Connection getAdminConnection(){
    	...
    	try{
    		...
    		st.execute("set @sessionkey='ABCD1234'");
    		st.close();
		} 
    	catch(Exception e){
			e.printStackTrace();
    ...
    //--- GET MEDICAL RECORD ----------------------------------------------------------------------
    public HealthRecordVO getMedicalRecord(String personid) {
    	PersonVO personVO = getPerson(personid);
        if(personVO == null)
            return null;
        PreparedStatement Occupstatement;
        Vector transactions = new Vector();
        ...
        try{
            OccupdbConnection = getOpenclinicConnection();
            Occupstatement = OccupdbConnection.prepareStatement("SELECT c.*,c.updateTime as ow_updateTime,a.start,a.stop,b.* from UsersView a, AdminView b, Transactions c,Healthrecord d where c.healthRecordId=d.healthRecordId and a.personid=b.personid and a.userid=c.userId and d.personId=? order by c.updateTime DESC,c.transactionId DESC");
            Occupstatement.setInt(1, personVO.getPersonId().intValue());
            ResultSet Occuprs;
            int transactionId, status, userId, personId, serverId, version, versionServerId;
            String transactionType, password, natreg, immatOld, immatNew, candidate, lastname, firstname, gender, language;
    ...
    public int getHealthRecordIdFromPersonIdWithCreate(int personId){
        ...
        try{
            ...
            if(Occuprs.next()){
            	...
            	Occuprs.close();
            	Occupstatement.close();
            	if(SH.ci("GDPREnabled", 0)==1) {
            		Occupstatement=OccupdbConnection.prepareStatement("insert into healthrecorddata(healthrecordid,datebegin,dateend,personid,serverid,version,versionserverid) values(aes_encrypt(?,'ABCD1234'),?,?,?,?,?,?)");
            	}
            	else {
    ...
    public void deleteTransaction(String transactionId, String serverId){
        ...
        if(tran!=null){            	
            if(tran.getTransactionType().equals(ScreenHelper.ITEM_PREFIX+"TRANSACTION_TYPE_ARCHIVE_DOCUMENT") || tran.getTransactionType().equals(ScreenHelper.ITEM_PREFIX+"TRANSACTION_TYPE_ARCHIVE_ADMINDOCUMENT")){
  ...
  //--- GET AGE ---------------------------------------------------------------------------------
    ...
    //--- GET ITEM --------------------------------------------------------------------------------
    public String getItemValue(int serverId, int transactionId, String itemType){
        String value = "";;
        Connection OccupdbConnection;
        try{
            OccupdbConnection = getOpenclinicConnection();
    ...
    public ItemVO getLastItemVOAfterWithHealthrecordId(int healthRecordId, String itemType, java.util.Date dAfter){
        ItemVO itemVO = null;
        Connection OccupdbConnection;
        try{
            OccupdbConnection = getOpenclinicConnection();
            PreparedStatement Occupstatement = OccupdbConnection.prepareStatement("SELECT * from Transactions a,Items b where a.healthRecordId=? and a.transactionId=b.transactionId and a.serverid=b.serverid and b.type = ? and a.updatetime>=? order by a.creationDate DESC,itemId DESC");
            Occupstatement.setInt(1, healthRecordId);
            Occupstatement.setString(2, itemType);
            Occupstatement.setTimestamp(3, new java.sql.Timestamp(dAfter.getTime()));
            ResultSet Occuprs = Occupstatement.executeQuery();
    ...
    public String getLastItemValueAfterWithHealthrecordId(int healthrecordId, String itemType, java.util.Date dAfter){
        ItemVO item = getLastItemVOAfterWithHealthrecordId(healthrecordId, itemType,dAfter);
        if(item!=null){
            return item.getValue();
        }
        return "";
...
//                var = "";
        ...
        try{
            ...
            if(rs.next()){
                ...
                if(rs.next()){
                    ...
                    healthRecordId = getOpenclinicCounter("HealthRecordID");
                	if(SH.ci("GDPREnabled", 0)==1) {
	                    sSelect = "insert into Healthrecorddata(healthRecordId,dateBegin,dateEnd,personId," +
	                            " serverid,version,versionserverid) values(aes_encrypt(?,'ABCD1234'),?,null,?,?,1,?)";
                	}
                	else {
                ...
                Integer newTransactionId = getOpenclinicCounter("TransactionID");
            ...
            for(Iterator iterator = transactionVO.getItems().iterator(); iterator.hasNext();){
                ...
                if(itemVO.getValue().trim().length() > 0){
                    ...
                    if(itemVO.getItemId().intValue() < 0){
                        itemVO.setItemId(getOpenclinicCounter("ItemID"));
                    }
                	Debug.println("itemVO.getItemId : "+itemVO.getItemId());
                	Debug.println("itemVO.getTransactionId : "+transactionVO.getTransactionId());
    ...
    public Vector getTransactionsForHealthrecordId(int healthRecordId){
        Vector transactionList = new Vector();
        Connection OccupdbConnection;
        String sSql = "SELECT a.* FROM Transactions a"+
                      " WHERE a.healthRecordId = ?"+
        		      " ORDER BY updateTime DESC";
