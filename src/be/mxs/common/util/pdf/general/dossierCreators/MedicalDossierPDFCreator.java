...
public class MedicalDossierPDFCreator extends PDFDossierCreator {
    ...
    protected void printActiveEncounter(AdminPerson activePatient) throws Exception {
		...
		if(activeEncounter!=null){
			...
			table.addCell(createItemValueCell(getTran(SH.cs("encounterOrigin","urgency.origin"),activeEncounter.getOrigin())));
		...
		if(visits.size() > 0){
			...
			for(int i=0; i<visits.size(); i++){
				...
				if(encounterUIDsToBePrinted.contains(encounter.getUid())){
					...
					encounterTable.addCell(createValueCell(getTran(SH.cs("encounterOrigin","urgency.origin"),encounter.getOrigin()),3));
