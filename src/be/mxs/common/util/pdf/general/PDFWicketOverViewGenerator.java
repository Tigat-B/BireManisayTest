...
public class PDFWicketOverViewGenerator extends PDFBasic {
    ...
    private void ventilateIncome(Hashtable incomeVentilation, Hashtable incomeVentilation2,WicketCredit wicketCredit){
		...
		if(wicketCredit.getInvoiceUID()==null && wicketCredit.getReferenceObject().getObjectType().equalsIgnoreCase("patientcredit")) {
			PatientCredit credit = PatientCredit.get(wicketCredit.getReferenceObject().getObjectUid());
			if(credit!=null && credit.getInvoiceUid()!=null) {
				wicketCredit.setInvoiceUID(credit.getInvoiceUid());
			}
		}
        if(wicketCredit.getInvoiceUID()!=null && wicketCredit.getInvoiceUID().length()>0 && wicketCredit.getInvoice()!=null && wicketCredit.getInvoice().getUid()!=null && wicketCredit.getInvoice().getUid().length()>0){
        	...
        	if(MedwanQuery.getInstance().getConfigInt("enableHMKIncomeVentilation",0)==1) {
                ...
                /*
	                ...
	                if(invoice!=null) {
	                	for(int n=0;n<prestations.size();n++) {
	                		Debet debet = (Debet)prestations.elementAt(n);
	    	                String ventilationService = "?";
	                		String su = debet.getServiceUid();
	                		if(su.length()>0) {
	                			ventilationService=su;
	                		}
	    	                if(incomeVentilation2.get(ventilationService)==null){
	    	                    incomeVentilation2.put(ventilationService,debet.getAmount()*wicketCredit.getAmount()/totalAmount);
	    	                }
	    	                else {
	    	                    incomeVentilation2.put(ventilationService,(Double)incomeVentilation2.get(ventilationService)+debet.getAmount()*wicketCredit.getAmount()/totalAmount);
	    	                }
	                	}
	                }
	            ...
	            for(int n=0; n<prestations.size();n++){
	                ...
	                String su=prestation.getServiceUid().toUpperCase();
