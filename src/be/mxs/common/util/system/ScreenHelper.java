...
public class ScreenHelper {
    ...
    public static String getLabelId(String labelType,String labelValue,String labelLanguage) {
    	String labelId="";
    	Connection conn = getOpenclinicConnection();
    	try {
    		PreparedStatement ps = conn.prepareStatement("select oc_label_id from oc_labels where oc_label_type=? and oc_label_value=? and oc_label_Language=?");
    		ps.setString(1, labelType);
    		ps.setString(2, labelValue);
    		ps.setString(3, labelLanguage);
    		ResultSet rs = ps.executeQuery();
    		if(rs.next()) {
    			labelId=rs.getString("oc_label_id");
    		}
    		rs.close();
    		ps.close();
    		ps.close();
        ...
        try{
            ...
            if(sType.equalsIgnoreCase("service") || sType.equalsIgnoreCase("function")){
                ...
                if(labelValue==null || labelValue.trim().length()==0){
                    ...
                    return "<span style='cursor: pointer;color: white;background-color: black;font-weight: bold' onclick=\"openPopup('"+url+"',800,100,'Edit');return false;\">"+(labelValue.length()>0?labelValue.substring(0, 1):"")+"</span>"+(labelValue.length()>1?labelValue.substring(1):""); // onclick without parenthesis
                }
            }
        }
        catch(Exception e){
...
public static String removeAccents(String sTest){
    ...
    public static String setNigerAdminPrivateContact(HttpServletRequest request,AdminPrivateContact apc, String sLanguage){
        String sCountry = "&nbsp;";
        if(checkString(apc.country).trim().length()>0){
            sCountry = getTran(null,"Country",apc.country,sLanguage);
        }
        ...
        return(
            ...
            setRow(request,"Web","region",apc.sanitarydistrict,sLanguage)+
            setRow(request,"Web","country.department",apc.district,sLanguage)+
            setRow(request,"Web","country.town",apc.sector,sLanguage)+
            setRow(request,"Web","village",apc.address,sLanguage)+
            setRow(request,"Web","postcode",apc.zipcode,sLanguage)+
            setRow(request,"Web","country",sCountry,sLanguage)+
            setRow(request,"Web","email",apc.email,sLanguage)+
            setRow(request,"Web","telephone",apc.telephone,sLanguage)+
            ...
            setRow(request,"Web","function",apc.businessfunction,sLanguage)+
            setRow(request,"Web","business",apc.business,sLanguage)+
            setRow(request,"Web","other",apc.comment,sLanguage)
        );
    }
    ...
    public static String writeDateField(String sName, String sForm, String sValue, boolean allowPastDates,
    	...
    	return writeDateField(sName,sForm,sValue,allowPastDates,allowFutureDates,sWebLanguage,sCONTEXTDIR,sExtraOnBlur,"");    	
	}
	
    public static String writeDateField(String sName, String sForm, String sValue, boolean allowPastDates, 
            boolean allowFutureDates, String sWebLanguage, String sCONTEXTDIR, String sExtraOnBlur, String sEvents){
		String gfPopType = "1"; // default        
		if(allowPastDates && allowFutureDates){
			gfPopType = "1";
		}
		...
		return "<span style='white-space: nowrap'><input type='text' maxlength='10' class='text' id='"+sName+"' name='"+sName+"' value='"+sValue+"' size='10' onblur='if(!checkDate(this)"+sExtraCondition+"){dateError(this);}else{"+sExtraOnBlur+"}' "+sEvents+"/>"+
    ...
    public static String writeDefaultCheckBox(TransactionVO transaction,HttpServletRequest request,String sLabelType, String sName, String sWebLanguage, String sEvent){
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
        	...
        	checkItemLabelTranslation(request.getSession(), sName, sLabelType);
    		StringBuffer s = new StringBuffer();
    		s.append("<input "+setRightClickMini(request.getSession(), sName, sLabelType)+" "+sEvent+" type='checkbox' id='"+sName+"' value='1'");
    ...
    public static String writeDefaultCountBoxes(TransactionVO transaction,HttpServletRequest request,String sLabelType, String sName, String sWebLanguage, boolean sorted, String sEvent,String separator,int count){
		boolean bMandatory=false;
		if(sName.startsWith("!")) {
			bMandatory=true;
			sName=sName.substring(1);
	        ...
	        if(labelTypes!=null){
	            ...
	            if(labelIds!=null){
	                ...
	                while(it.hasNext()){
	                    ...
	                    s.append("<label style='display: inline-block;'>");
	                    s.append("<select "+(bMandatory?"data-mandatory='1'":"")+" onchange='removeOutline(this);updateMultiCountbox(this,\""+sName+"\",\""+sLabelID+"\")' class='text' "+(checkString((String)request.getSession().getAttribute("editmode")).equalsIgnoreCase("1")?"title='"+sName+"' ":"")+"name='"+sName+"."+sLabelID+"' id='"+sName+"."+sLabelID+"'>");
	                    s.append("<option/>");
	                    for(int n=1;n<=count;n++) {
	                    	s.append("<option value='"+n+"'");
		                    if(arrayContains(sSelected,sLabelID+"="+n,";") || (n==1 && arrayContains(sSelected,sLabelID,";"))){
		                        s.append(" selected");
		                    }
	                    	s.append(">"+n+"</option>");
	                    }
	                    ...
	                    s.append(sLabelValue+" "+(it.hasNext()?separator:"")+"</label>");
	                }
	            }
	        }
	    	return s.toString();
    ...
    public static String writeDefaultLastDateInput(HttpServletRequest request, TransactionVO transaction, String sName, String language,String contextpath){
    	...
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
	    	...
	    	s.append(" class='text' size='12' maxlength='10' name='currentTransactionVO.items.<ItemVO[hashCode="+transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getItemId()+"]>.value'");
	    	s.append(" onblur='checkDate(this);' value='"+SH.getLastItem(request,"be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue()+"'/>");
	    	s.append("<script>writeMyDate('"+sName+"', '"+contextpath+"/_img/icons/icon_agenda.png', '"+ScreenHelper.getTranNoLink("web","PutToday",language)+"');</script>");
	    	return s.toString();
    	}
    }
    ...
    public static String writeDefaultLastDateInput(HttpServletRequest request, TransactionVO transaction, String sName, String language,String contextpath, String event){
    	HttpSession session=request.getSession();
    	boolean bMandatory=false;
    	if(sName.startsWith("!")) {
    		sName=sName.substring(1);
    	...
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
	    	...
	    	s.append(" class='text' size='12' maxlength='10' name='currentTransactionVO.items.<ItemVO[hashCode="+transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getItemId()+"]>.value'");
	    	s.append(" onblur='checkDate(this);' "+event+" value='"+SH.getLastItem(request,"be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue()+"'/>");
	    	s.append("<script>writeMyDate('"+sName+"', '"+contextpath+"/_img/icons/icon_agenda.png', '"+ScreenHelper.getTranNoLink("web","PutToday",language)+"');</script>");
	    	return s.toString();
    	}
    }
    ...
    public static String writeDefaultSelect(HttpServletRequest request,TransactionVO transaction, String sName,String type, String language, String sEvent,String sDefault){
    	...
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
        	...
        	checkItemLabelTranslation(request.getSession(), sName, type);
	    	StringBuffer s = new StringBuffer();
  ...
  public static String writeSelectWithStyle(HttpServletRequest request,String sLabelType, String sSelected,String sWebLanguage, String sStyle){
        ...
        if(labelTypes!=null){
            ...
            if(labelIds!=null){
                ...
                while(it.hasNext()){
                    ...
                    sOptions+= "/><label "+setEditTitle(request.getSession(), sBaseName, sLabelType)+" for='"+sBaseName+"-"+sLabelID+"'>"+sLabelValue+"</label>";
                }
            }
        }
        if(request!=null && checkString((String)request.getSession().getAttribute("editmode")).equalsIgnoreCase("1")){
    ...
    public static String checkString(Object sString, String defaultValue){
        // om geen 'null' weer te geven
        if((sString==null)||(sString.toString().toLowerCase().equals("null"))||sString.toString().length()==0){
            return defaultValue;
        }
        else{
            sString = sString.toString().trim();
        }
    ...
    public static String formatDate(java.util.Date dDate, String dateFormat){
        String sDate = "";
        if(dDate!=null){
            sDate = new SimpleDateFormat(dateFormat).format(dDate);
        }
        return sDate;
    }
    ...
    public static String setRightClickMini(HttpSession session,String itemType, String labelType){
        return "onmouseover=\"if(document.getElementById('clipboard')) document.getElementById('clipboard').innerHTML='"+itemType+"';this.style.cursor='help';setPopup('be.mxs.common.model.vo.healthrecord.IConstants."+itemType+"',this.value);activeItem=true;setItemsMenu(true);\" "+
               "onmouseout=\"this.style.cursor='default';activeItem=false;document.oncontextmenu=function(){return true};\" "+
               (checkString((String)session.getAttribute("editmode")).equals("1")?"title='"+itemType+" ["+labelType+"]' ":"");
    }

    public static void checkItemLabelTranslation(HttpSession session, String itemType, String labelType) {
    	if(SH.c(labelType).length()>0 && checkString((String)session.getAttribute("editmode")).equals("1") && checkString((String)session.getAttribute("activeTransactionType")).length()>0) {
    		String transactionType=checkString((String)session.getAttribute("activeTransactionType"));
    		//Store the translation value for this transactionItem
    		Connection conn = SH.getOpenClinicConnection();
    		try {
				PreparedStatement ps = conn.prepareStatement("select modifier from transactionitems where transactiontypeid=? and itemtypeid=?");
				ps.setString(1, transactionType);
				ps.setString(2, SH.ITEM_PREFIX+itemType);
				ResultSet rs = ps.executeQuery();
				if(rs.next()) {
					String[] modifiers = SH.c(rs.getString("modifier")).split(";");
					String newModifiers ="";
					if(modifiers.length<2) {
						newModifiers=";"+labelType;
					}
					else if(!modifiers[1].equalsIgnoreCase(labelType)) {
						for(int n=0;n<modifiers.length;n++) {
							if(n==1) {
								newModifiers+=labelType+";";
							}
							else {
								newModifiers+=modifiers[n]+";";
							}
						}
					}
					rs.close();
					...
					if(newModifiers.length()>0) {
						ps=conn.prepareStatement("update transactionitems set modifier=? where transactiontypeid=? and itemtypeid=?");
						ps.setString(1, newModifiers);
						ps.setString(2, transactionType);
						ps.setString(3, SH.ITEM_PREFIX+itemType);
						ps.execute();
						ps.close();
					}
				}
    ...
    public static String setEditTitle(HttpSession session,String itemType, String labelType){
    	checkItemLabelTranslation(session, itemType, labelType);
        return (checkString((String)session.getAttribute("editmode")).equals("1")?"title='"+itemType+" ["+labelType+"]' ":"");
    }
