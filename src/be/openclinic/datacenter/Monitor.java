...
import be.opencarenet.OpenCarenetClient;
...
public class Monitor implements Runnable{
	...
	public static void runScheduler(){
        ...
        try {
    		...
    		if(MedwanQuery.getInstance().getConfigString("ghb_ref_serverid","").trim().length()>0 && new java.util.Date().getTime()-lastGHBMessageSent.getTime()>=1*minute){
				Debug.println("=================>>>>>>>>>>>>>>>>>> Syncing GHB messages");
    		...
    		Date lastGHBServersSync = new SimpleDateFormat("yyyyMMddHHmmss").parse(MedwanQuery.getInstance().getConfigString("lastGHBServersSync","19000101010000"));
    		if(MedwanQuery.getInstance().getConfigString("ghb_ref_serverid","").trim().length()>0 && new java.util.Date().getTime()-lastGHBServersSync.getTime()>=60*minute){
				Debug.println("=================>>>>>>>>>>>>>>>>>> Syncing GHB servers");
    			Debug.println(GHBNetwork.syncGHBServers(MedwanQuery.getInstance().getConfigString("ghb_ref_projectdomain","")));
   				MedwanQuery.getInstance().setConfigString("lastGHBServersSync",new SimpleDateFormat("yyyyMMddHHmmss").format(new java.util.Date()));
    		}
            //Waiting queue stats
            try {
	...
	public void run() {
		...
		Debug.println("==============>>>>>>>>>>>>>>> Running monitor UID "+uid);
        try {
        	while(!isStopped()){
        		//First check if no other active broker is running on this system
        		Debug.println("runScheduler="+SH.cs("activeMonitor", uid+";"+new java.util.Date().getTime()));
        		...
        		if(activeMonitor.split(";")[0].equals(uid) || new java.util.Date().getTime()- Long.parseLong(activeMonitor.split(";")[1]) > 2*SH.ci("globalhealthbarometerMonitorInterval",200000)) {
	        		...
	        		if(SH.ci("enableOpenCarenet",0)==1){
		                //Get OpenCarenetMessages
		                try {
		        			Date dLastOpenCarenet = new SimpleDateFormat("yyyyMMddHHmmss").parse(MedwanQuery.getInstance().getConfigString("lastOpenCarenetConnect","19000101010000"));
		        			long interval = MedwanQuery.getInstance().getConfigInt("openCarenetInterval",3600)*1000; //default = once an hour
		        			if(new java.util.Date().getTime()-dLastOpenCarenet.getTime()>interval){
		        				OpenCarenetClient client = new OpenCarenetClient();
		        				int minMessage=0;
		        				try {
		        					minMessage=Integer.parseInt(Pointer.getPointer("OPENCARENET.MAXMESSAGEID"));
		        				}
		        				catch(Exception es) {
		        					es.printStackTrace();
		        				}
		        				try {
		        					Debug.println("<<< --- Checking OpenCarenet messages...");
		        					client.readMessages(minMessage);
		        					Debug.println("...done --- >>>");
		        				}
		        				catch(Exception o) {
		        					SH.syslog("Error retrieving messages from OpenCarenet server");
		        					Debug.println(o.getStackTrace().toString());
		        				}
		        				MedwanQuery.getInstance().setConfigString("lastOpenCarenetConnect",new SimpleDateFormat("yyyyMMddHHmmss").format(new java.util.Date()));
		        			}
		        		} catch (Exception e) {
		        			e.printStackTrace();
		        		}
