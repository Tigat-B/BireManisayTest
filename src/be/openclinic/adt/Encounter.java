...
public class Encounter extends OC_Object {
	...
	public Vector<be.openclinic.medical.Diagnosis> getDiagnoses(String codetype) {
		Vector<be.openclinic.medical.Diagnosis> diags = new Vector<be.openclinic.medical.Diagnosis>();
		for(int n=0;n<diagnoses.size();n++) {
			be.openclinic.medical.Diagnosis diagnosis = diagnoses.elementAt(n);
			if(diagnosis.getCodeType().equalsIgnoreCase(codetype)) {
				diags.add(diagnosis);
			}
		}
		return diags;
	...
	public void setResponsibleService(String n){
		setModifier(9,n+"");
	}
    ...
    public static boolean isNewCase(String encounterUid){
    	if(encounterUid==null || encounterUid.split("\\.").length<2) {
    		return false;
    	}
    ...
    public static Encounter get(String uid, int version) {
    	Encounter encounter = new Encounter();
        String sSelect = "SELECT * FROM OC_ENCOUNTERS_HISTORY WHERE OC_ENCOUNTER_SERVERID = ? AND OC_ENCOUNTER_OBJECTID = ? and OC_ENCOUNTER_VERSION=?";
    	...
    	if(!encounter.hasValidUid()) {
    		encounter=get(uid);
    		if(encounter!=null && encounter.getVersion()==version) {
    			return encounter;
    		}
    	}
    	return encounter;
    ...
    public String getServiceUID(Date date){
        try{
            ...
            for(int n=0; n<encounterServices.size(); n++){
                ...
                Date begin = encounterService.begin,
                     end   = encounterService.end;
