			...
			if(service!=null && ScreenHelper.checkString(service.costcenter).length()>0 ){
				...
				while(rs.next()){
					...
					for(int n=0;n<noms.length;n++){
				    	...
				    	if(service.getParentcode().length()==0) {
				    		ps2 = conn.prepareStatement("select count(*) total from oc_assets where (oc_asset_service=? OR oc_asset_service like ?) and (oc_asset_nomenclature=? or oc_asset_nomenclature like ?) and (oc_asset_saledate is null OR oc_asset_saledate>?) and (oc_asset_comment7 is null or oc_asset_comment7='' or oc_asset_comment7<3) and (oc_asset_comment12 is NULL or length(oc_asset_comment12)=0 or STR_TO_DATE(oc_asset_comment12,'%d/%m/%Y')<=?)");
				    	}
						ps2.setString(1, serviceid);
				    	if(service.getParentcode().length()==0) {
