...
public class Asset extends OC_Object {
    ...
    public static void updateAsset(Element eAsset) throws Exception{
		...
		if(!rs.next()){
			...
			asset.setComment21(eAsset.element("comment21")==null?"":eAsset.element("comment21").getText());
			asset.setComment22(eAsset.element("comment22")==null?"":eAsset.element("comment22").getText());
			asset.setComment23(eAsset.element("comment23")==null?"":eAsset.element("comment23").getText());
			asset.setComment24(eAsset.element("comment24")==null?"":eAsset.element("comment24").getText());
			asset.setComment25(eAsset.element("comment25")==null?"":eAsset.element("comment25").getText());
    ...
    public static StringBuffer toXml(String uid){
    	...
    	try{
    		...
    		if(rs.next()){
    			...
    			addTagBinaryString(xml,"comment21",rs);
    			addTagBinaryString(xml,"comment22",rs);
    			addTagBinaryString(xml,"comment23",rs);
    			addTagBinaryString(xml,"comment24",rs);
    			addTagBinaryString(xml,"comment25",rs);
    ...
    public static Vector getDefaultAssets(String sNomenclatureCode) {
        ...
        try{
        	...
        	while(rs.next()) {
                ...
                asset.setComment21(rs.getString("OC_ASSET_COMMENT21"));
                asset.setComment22(rs.getString("OC_ASSET_COMMENT22"));
                asset.setComment23(rs.getString("OC_ASSET_COMMENT23"));
                asset.setComment24(rs.getString("OC_ASSET_COMMENT24"));
                asset.setComment25(rs.getString("OC_ASSET_COMMENT25"));
                assets.add(asset);
        	}
        	rs.close();
        	ps.close();
        ...
        try{            
            if(getUid().equals("-1")){                  
                ...
                sSql = "INSERT INTO oc_assets(OC_ASSET_SERVERID,OC_ASSET_OBJECTID,OC_ASSET_CODE,"+
                       ...
                       + ",OC_ASSET_COMMENT21,OC_ASSET_COMMENT22,OC_ASSET_COMMENT23,OC_ASSET_COMMENT24,OC_ASSET_COMMENT25)"+
                       " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"; // 31
                ps = oc_conn.prepareStatement(sSql);
                ...
                ps.setString(psIdx++,comment21);
                ps.setString(psIdx++,comment22);
                ps.setString(psIdx++,comment23);
                ps.setString(psIdx++,comment24);
                ps.setString(psIdx++,comment25);
                ...
                sSql = "UPDATE oc_assets SET"+
                       ...
                       "  , OC_ASSET_COMMENT20=?, OC_ASSET_SERVICE=?, OC_ASSET_LOCKEDBY=?, OC_ASSET_LOCKEDDATE=?"+
                       "  , OC_ASSET_COMMENT21=?, OC_ASSET_COMMENT22=?, OC_ASSET_COMMENT23=?, OC_ASSET_COMMENT24=?, OC_ASSET_COMMENT25=?"+// update-info
    ...
    public static List<Asset> getList(Asset findItem){
        ...
        try{
            ...
            if(ScreenHelper.checkString(findItem.serviceuid).length() > 0){
            	Service service = Service.getService(findItem.serviceuid);
            	if(service.getParentcode().length()>0) {
    ...
    */    
    ...
    public String getSupplierName(String sSupplierUid){
        if(ScreenHelper.checkString(sSupplierUid).length()==0 || sSupplierUid.split("\\.").length<2){
        	return "";
        }
