...
public class ArchiveDocument extends OC_Object implements Comparable {
	...
	*/
	...
	public static ArchiveDocument getThroughTransactions(String sUDI, int personId) throws Exception {
		...
		allArchDocs = MedwanQuery.getInstance().getTransactionsByType(personId,ScreenHelper.ITEM_PREFIX+"TRANSACTION_TYPE_ARCHIVE_DOCUMENT");
		docIter = allArchDocs.iterator();
		while(docIter.hasNext()){
			tran = (TransactionVO)docIter.next();
	...
	public static Vector<ArchiveDocument> getAllByReference(String sReference){
		...
		String sSql = "SELECT * FROM arch_documents"+
		              " WHERE ARCH_DOCUMENT_REFERENCE like ?";
	...
	public static Vector<ArchiveDocument> getActiveByReferences(Vector<String> references){
		Vector docs = new Vector();
		ArchiveDocument archDoc = null;
		...
		for(int n=0;n<references.size();n++) {
			refs+=",'"+references.elementAt(n)+"'";
		}
		SH.syslog(refs);
		
		String sSql = "SELECT * FROM arch_documents"+
		              " WHERE ARCH_DOCUMENT_REFERENCE in ("+refs+") and arch_document_deletedate is null";
	...
	public static Vector getMostRecentDocuments(int maxRecords, int personId, boolean onlyScannedDocuments) throws Exception {
		...
		allArchDocs = MedwanQuery.getInstance().getTransactionsByType(personId,
                ScreenHelper.ITEM_PREFIX+"TRANSACTION_TYPE_ARCHIVE_ADMINDOCUMENT");
		allArchDocs.iterator();
		while(docIter.hasNext() && docCount < maxRecords){	
			tran = (TransactionVO)docIter.next();
