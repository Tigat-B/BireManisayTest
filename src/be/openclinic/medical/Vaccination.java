    ...
    public static String getVaccination(Hashtable vaccinations,long age,HttpServletRequest request,String type,String parameter){
		...
		if(vaccination!=null){
			if(parameter.equalsIgnoreCase("date")){
				...
				sResult=ScreenHelper.checkString(vaccination.location,"#").split(":")[0];
				if(sResult.startsWith("0")){
					...
					sResult=ScreenHelper.getTranNoLink("vaccinationlocation",sResult,((User)request.getSession().getAttribute("activeUser")).person.language);
				}
			}
		}
