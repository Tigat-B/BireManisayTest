...
public class Diagnosis extends OC_Object{
    ...
    //--- DELETE FOR ENCOUNTER --------------------------------------------------------------------
    public static void deleteForEncounter(String encounterUid, String codetype, String code){
        PreparedStatement ps;
        Connection oc_conn = MedwanQuery.getInstance().getOpenclinicConnection();
        try{
            ps = oc_conn.prepareStatement("delete from OC_DIAGNOSES where OC_DIAGNOSIS_ENCOUNTERUID=? and OC_DIAGNOSIS_CODETYPE=? and OC_DIAGNOSIS_CODE=?");
            ps.setString(1,encounterUid);
    ...
    public static String getCodeLabel(String code, String codeType, String language){
        ...
        String sSelect = "";
        if(codeType.replaceAll("-","").equalsIgnoreCase("ICPC2") || codeType.replaceAll("-","").equalsIgnoreCase("ICPC")){
            sSelect = "SELECT "+label+" from ICPC2 WHERE code = ?";
        }
        else if(codeType.replaceAll("-","").equalsIgnoreCase("ICD10")||codeType.replaceAll("-","").equalsIgnoreCase("ICD")){
    ...
    public static Hashtable getDiagnosesByReferenceUID(String sReferenceUID,String sReferenceType){
        ...
        try{
            ...
            while(rs.next()){
                ...
                hDiagnosisInfo.put("ServerId",ScreenHelper.checkString(rs.getString("OC_DIAGNOSIS_SERVERID")));
                hDiagnosisInfo.put("ObjectId",ScreenHelper.checkString(rs.getString("OC_DIAGNOSIS_OBJECTID")));
