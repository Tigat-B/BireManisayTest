...
public class Invoice extends OC_Object {
   ...
   public HashSet getEncounters(){
	   HashSet encounters = new HashSet();
	   for(int n=0;n<debets.size();n++){
		   Debet debet = (Debet)debets.elementAt(n);
		   if(ScreenHelper.checkString(debet.getEncounterUid()).length()>0){
   ...
   public Vector getDiagnoses(String codeType,int significance) {
	    Vector diagnoses = new Vector();
	  	...
	  	for(int n=0;n<debets.size();n++) {
	  		...
	  		if(!encounters.contains(debet.getEncounterUid())) {
	  			Vector diags = Diagnosis.selectDiagnoses("","",debet.getEncounterUid(),"","","","","","","","",codeType,"");
	  			for(int i=0;i<diags.size();i++) {
	  				Diagnosis diag = (Diagnosis)diags.elementAt(i);
	  				String cd = diag.getCode();
	  				if(significance>0 && cd.length()>significance) {
	  					cd=cd.substring(0,significance);
	  				}
	  				diagnoses.add(cd);
	  			}
	  			encounters.add(debet.getEncounterUid());
   ...
   public HashSet getInsurances(){
	   HashSet insurances = new HashSet();
	   for(int n=0;n<debets.size();n++){
		   Debet debet = (Debet)debets.elementAt(n);
		   if(ScreenHelper.checkString(debet.getInsuranceUid()).length()>0){
			   insurances.add(debet.getInsuranceUid());
		   }
	   }
	   return insurances;
   }
