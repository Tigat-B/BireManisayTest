...
public class PatientInvoice extends Invoice {
    ...
    protected String debetsString;
	protected String creditsString;
	...
	public String getDebetsString() {
		return debetsString;
	}
	public void setDebetsString(String debetsString) {
		this.debetsString = debetsString;
	}
	public String getCreditsString() {
		return creditsString;
	}
	public void setCreditsString(String creditsString) {
		this.creditsString = creditsString;
	}
	
	public java.util.Date getFirstDebetDate(){
		java.util.Date date = null;
		for(int n=0;n<getDebets().size(); n++) {
			Debet debet=(Debet)getDebets().elementAt(n);
			if(date==null || debet.getDate().before(date)) {
				date=debet.getDate();
			}
		}
		return date;
	...
	public java.util.Date getLastDebetDate(){
		java.util.Date date = null;
		for(int n=0;n<getDebets().size(); n++) {
			Debet debet=(Debet)getDebets().elementAt(n);
			if(date==null || debet.getDate().after(date)) {
				date=debet.getDate();
			}
		}
		return date;
    ...
    //--- GET -------------------------------------------------------------------------------------
    public static PatientInvoice get(String uid, int version) {
    	PatientInvoice patientInvoice = new PatientInvoice();
        String sSelect = "SELECT * FROM OC_PATIENTINVOICES_HISTORY WHERE OC_PATIENTINVOICE_SERVERID = ? AND OC_PATIENTINVOICE_OBJECTID = ? and OC_PATIENTINVOICE_VERSION=?";
        Connection oc_conn=SH.getOpenclinicConnection();
        try{
            ...
            if(rs.next()){
                ...
                patientInvoice.setCreditsString(rs.getString("OC_PATIENTINVOICE_CREDITS"));
                patientInvoice.setDebetsString(rs.getString("OC_PATIENTINVOICE_DEBETS"));
                patientInvoice.debets = new Vector();
                String[] sDebets = SH.c(patientInvoice.getDebetsString()).split(";");
                for(int n=0;n<sDebets.length;n++) {
                	if(sDebets[n].split(":").length>1) {
                		patientInvoice.debets.add(Debet.get(sDebets[n].split(":")[0],Integer.parseInt(sDebets[n].split(":")[1])));
                	}
                	else if(sDebets[n].length()>0) {
                		patientInvoice.debets.add(Debet.get(sDebets[n]));
                	}
                }
                patientInvoice.credits = new Vector();
                String[] sCredits = SH.c(patientInvoice.getCreditsString()).split(";");
                for(int n=0;n<sCredits.length;n++) {
                	if(sCredits[n].length()>0) {
                		patientInvoice.credits.add(sCredits[n].split(":")[0]);
                	}
                }
            }
            rs.close();
    	...
    	if(!patientInvoice.hasValidUid()) {
    		patientInvoice=get(uid);
    		if(patientInvoice!=null && patientInvoice.getVersion()==version) {
                patientInvoice.setCreditsString(patientInvoice.getCreditsArray());
                patientInvoice.setDebetsString(patientInvoice.getDebetsArray());
    ...
    public Vector getCreditStrings(){
        Vector d = new Vector();
        
        if(credits!=null){
            for(int n=0;n<credits.size();n++){
                d.add(PatientCredit.get((String)credits.elementAt(n)).getUid());
            }
        }
    ...
    public String getDebetsArray() {
    	StringBuffer s= new StringBuffer();
        Vector d = new Vector();
        if(debets!=null){
            for(int n=0;n<debets.size();n++){
            	Debet debet = (Debet)debets.elementAt(n);
                d.add(debet.getUid()+":"+debet.getVersion());
            }
        }
    	Collections.sort(d);
    ...
    public String getCreditsArray() {
        ...
        if(credits!=null){
            for(int n=0;n<credits.size();n++){
            	PatientCredit credit = PatientCredit.get((String)credits.elementAt(n));
                d.add(credit.getUid()+":"+credit.getVersion());
            }
        }
    	Collections.sort(d);
    	for(int n=0;n<d.size();n++) {
        ...
        try{
            if(this.getUid()!=null && this.getUid().length() > 0){
                ...
                if(ids.length == 2){
                    ...
                    rs.close();
                    ps.close();
                    String columns=	"OC_PATIENTINVOICE_SERVERID," +
		                            ...
		                            " OC_PATIENTINVOICE_DEBETS,"+
		                            " OC_PATIENTINVOICE_CREDITS";

                    sSelect = "INSERT INTO OC_PATIENTINVOICES_HISTORY("+columns+") SELECT "+columns+" FROM OC_PATIENTINVOICES WHERE OC_PATIENTINVOICE_SERVERID = ? AND OC_PATIENTINVOICE_OBJECTID = ?";
                    ps = oc_conn.prepareStatement(sSelect);
                    ps.setInt(1,Integer.parseInt(ids[0]));
                    ps.setInt(2,Integer.parseInt(ids[1]));
                    ps.executeUpdate();
            ...
            if(ids.length == 2){
                ...
                ps.setString(19, getDebetsArray());
                ps.setString(20, getCreditsArray());
                ps.executeUpdate();
                ps.close();
    ...
    public static Vector searchInvoicesCreated(java.util.Date begin, java.util.Date end, String sPatientUid){
        Vector invoices = new Vector();
        PreparedStatement ps = null;
        ResultSet rs = null;
        Connection oc_conn=MedwanQuery.getInstance().getOpenclinicConnection();
        try {
        	String sql = "select * from oc_patientinvoices where oc_patientinvoice_createtime>=? and oc_patientinvoice_createtime<=? and oc_patientinvoice_patientuid=?";
        	...
        	rs=ps.executeQuery();
        	while(rs.next()) {
        		invoices.add(PatientInvoice.get(rs.getString("oc_patientinvoice_serverid")+"."+rs.getString("oc_patientinvoice_objectid")));
        	}
        	rs.close();
        	ps.close();
        	oc_conn.close();
    ...
    public static double getTotalBalanceForPatient(String sInvoicePatientUid, java.util.Date dInvoiceDateFrom, java.util.Date dInvoiceDateTo){
        PreparedStatement ps = null;
        ResultSet rs = null;
        double totalBalance = 0;
        ...
        try{                      
            ...
            if(dInvoiceDateFrom!=null){
                if(sSql.indexOf("WHERE")<0) sSql+= " WHERE";
                sSql+= " OC_PATIENTINVOICE_UPDATETIME >= ? AND";
            }
            ...
            sSql+=" ORDER BY OC_PATIENTINVOICE_UPDATETIME DESC";
            ...
            SimpleDateFormat stdDateFormat = ScreenHelper.stdDateFormat;
            if(sInvoicePatientUid.length() > 0) ps.setString(qmIdx++,sInvoicePatientUid);
            if(dInvoiceDateFrom!=null) ps.setTimestamp(qmIdx++,SH.getSQLTimestamp(dInvoiceDateFrom));
            if(dInvoiceDateTo!=null) ps.setTimestamp(qmIdx++,SH.getSQLTimestamp(dInvoiceDateTo));
