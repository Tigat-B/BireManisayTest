    ...
    <ul class="nav-list" role="navigation">
      <div class="list -left">
        ...
        <li class="item"  style='display: <%=checkString(activeUser.getParameter("onetime")).length()>0?"none":""%>'>
      	...
      	<%if(checkString(activeUser.getParameter("patientid")).length()==0 && activeUser.getAccessRightNoSA("mpi.serveradmin.select")){ %>
        <li class="item">
          <a style='text-decoration: none;font-family: Raleway, Geneva, sans-serif;' class="link" href="<%=sCONTEXTPATH %>/main.jsp?Page=system/manageOpenCarenetTraffic.jsp"><img height='24px' style='vertical-align: middle' src='<%=sCONTEXTPATH%>/_img/icons/mobile/icon_config.png'/> <%=getTranNoLink("web","administration",sWebLanguage) %></a>
        </li>
        <%} %>
        <li class="item">
