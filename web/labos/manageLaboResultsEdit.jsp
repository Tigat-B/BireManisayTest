...
<%
    ...
    if (request.getParameter("submit") != null) {
        ...
        while (e.hasMoreElements()) {
            ...
            else if (name.startsWith("rejected.")) {
                String[] v = name.split("\\.");
                String value = request.getParameter(name);
                if(value.length()>0){
	                RequestedLabAnalysis.updateValue(Integer.parseInt(v[1]), Integer.parseInt(v[2]), v[3], "@@REJECTED",Integer.parseInt(activeUser.userid));
	                RequestedLabAnalysis.setModifiedFinalValidation(Integer.parseInt(v[1]), Integer.parseInt(v[2]), Integer.parseInt(activeUser.userid), "'"+v[3]+"'",value);
	                RequestedLabAnalysis.updateResultComment(Integer.parseInt(v[1]), Integer.parseInt(v[2]), v[3], value.split(";").length<2?"":value.split(";")[1]);
                }
            } 
...
<%=writeTableHeader("Web", "openPatientLaboResults", sWebLanguage, " doBack();")%><form method='post' name='fastresults'>
                    ...
                    if(analysisCode.startsWith("00SCAN00")){
	                    ...
	                    while (requestsIterator.hasNext()) {
	                        ...
	                        if(requestedLabAnalysis != null){
		                        ...
		                        String reject = "<span id='rejectcomment."+labRequest.getServerid()+"."+labRequest.getTransactionid()+"."+requestedLabAnalysis.getAnalysisCode()+"'></span><input type='hidden' id='rejected."+labRequest.getServerid()+"."+labRequest.getTransactionid()+"."+requestedLabAnalysis.getAnalysisCode()+"' name='rejected."+labRequest.getServerid()+"."+labRequest.getTransactionid()+"."+requestedLabAnalysis.getAnalysisCode()+"' value=''/><img id='rejectimg."+labRequest.getServerid()+"."+labRequest.getTransactionid()+"."+requestedLabAnalysis.getAnalysisCode()+"' style='vertical-align: middle' height='14px' title='"+getTranNoLink("web","rejectsample",sWebLanguage)+"' src='"+sCONTEXTPATH+"/_img/icons/mobile/delete.png' onclick='rejectsample(\""+labRequest.getServerid()+"."+labRequest.getTransactionid()+"."+requestedLabAnalysis.getAnalysisCode()+"\")'/>";
		                        if(requestedLabAnalysis.getResultValue().equalsIgnoreCase("@@REJECTED")){
		                        	result=getTranNoLink("web","rejected",sWebLanguage)+" "+requestedLabAnalysis.getResultComment();
		                        }
		                        else{
		                        	result=result+" "+reject;
		                        }
	                        }
	                        boolean bAbnormal = (result.length() > 0 && !result.equalsIgnoreCase("?") && !result.equalsIgnoreCase("@@REJECTED") && abnormal.toLowerCase().indexOf("*"+checkString(requestedLabAnalysis.getResultModifier()).toLowerCase()+"*") > -1);
    ...
    <input type='hidden' id='samplerejectioncomment' value=''/>
    <p/>
...
<script>
    ...
    function rejectsample(id){
		if(window.confirm('<%=getTranNoLink("web","areyousuretorejectsample",sWebLanguage)%>')){
			var comment = window.prompt('<%=getTranNoLink("web","provideoptionalcomment",sWebLanguage)%>',document.getElementById('samplerejectioncomment').value);
			document.getElementById('rejected.'+id).value='@@REJECTED;'+comment;
			document.getElementsByName('result.'+id)[0].value='';
			if(document.getElementsByName('resultcomment.'+id) && document.getElementsByName('resultcomment.'+id)[0]){
				document.getElementsByName('resultcomment.'+id)[0].value='';
			}
			document.getElementsByName('result.'+id)[0].style.display='none';
			document.getElementById('rejectimg.'+id).style.display='none';
			document.getElementById('rejectcomment.'+id).innerHTML='<b><%=getTranNoLink("web","rejected",sWebLanguage)%> '+comment+"</b>";
			document.getElementById('samplerejectioncomment').value=comment;
		}
    }
