    ...
    public String contextHeader(HttpServletRequest request, String language, boolean setBackGround, String subTitle){
        ...
        try{
            ...
            if(transactionVO!=null){
                request.getSession().setAttribute("activeTransactionType", transactionVO.getTransactionType());
                ...
                if(transactionVO.getTransactionType().indexOf("_CUSTOMEXAMINATION") > -1){
                	...
                	if(request.getSession().getAttribute("editmode")!=null && request.getSession().getAttribute("editmode").equals("1")){
                		result+="<input type='button' class='button' onclick='translateItems(\""+transactionVO.getTransactionType()+"\")' value='"+getTranNoLink("web","itemtranslations",language)+"'/>&nbsp;&nbsp;";
                	}
            ...
            result+= "<td align='right'>"+
                      "<select id='ctxt' class='text' onchange=\""+(itemVO==null?"":"document.getElementsByName('currentTransactionVO.items.<ItemVO[hashCode="+itemVO.getItemId()+"]>.value')[0].value=this.value;show('content-details');if($('confirm'))hide('confirm');")+"\" style='display:none'>";
    ...
    public String getButtonsHtml(HttpServletRequest req, User activeUser, AdminPerson activePatient,
        ...
        if(sAccessRight.length()==0 || !sAccessRight.equalsIgnoreCase("readonly") && ((activeUser.getParameter("sa")!=null && activeUser.getParameter("sa").length() > 0) || activeUser.getAccessRight(sAccessRight+".add") || activeUser.getAccessRight(sAccessRight+".edit"))){
            ...
            if(SH.ci("enableOfflineSync",0)==0 || bIsNew ){
	            if(tran.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_REFERRAL_SOURCESITE").length()==0){
		            ...
		            if(MedwanQuery.getInstance().getConfigInt("enableArmyWeek",0)==0){
			            ...
			            if(MedwanQuery.getInstance().getConfigString("ghb_ref_serverid","").trim().length()>0 && activeUser.getAccessRight("system.sendrecord.select")){
