    ...
    <tbody id="invoicedetails" style="visibility:hidden">
        ...
        <tr id="period" style="visibility:hidden">
            ...
            <td class="admin2">
                ...
                %>                        
                &nbsp;<input type="button" class="button" name="updateDebets" id="updateDebets" value="<%=getTranNoLink("web","update",sWebLanguage)%>" onclick="changeInsurar();"/>
            </td>
        </tr>
    ...
    for(i=0; i<images.length; i++){
      ...
      if(elm.name.indexOf('cbDebet') > -1){
        if(elm.src.indexOf('/check.gif')>0){
        	sCbs+= elm.name.split("=")[0].replace("cbDebet","d")+",";
        }
      }
    }
    for(i=0; i<EditForm.elements.length; i++){
      elm = EditForm.elements[i];
      if(elm.name.indexOf('cbInsurarInvoice') > -1 && elm.checked){
          sCbs+= elm.name.split("=")[0].replace("cbInsurarInvoice","c")+",";
      }
    }
