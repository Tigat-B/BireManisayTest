...
<form name="EditForm" method="POST" action="<c:url value='/main.do'/>?Page=financial/wicket/manageWicketOperationDebet.jsp&ts=<%=getTs()%>">
    <table class='list' border='0' width='100%' cellspacing='1' cellpadding="0">
        ...
        <tr>
            ...
            <% 	if(SH.ci("allowWicketDebetDateChange",1)==1){ %>
            		<td class="admin2"><%=SH.writeDateField("EditWicketOperationDate","EditForm",sEditWicketOperationDate,true,false,sWebLanguage,sCONTEXTPATH)%></td>
            <%	}
            	else {	
            %>
            		<td class="admin2"><input type='text' class='text' size='10' name='EditWicketOperationDate' id='EditWicketOperationDate' value='<%=sEditWicketOperationDate%>' readonly/></td>
            <%	} %>
        </tr>
