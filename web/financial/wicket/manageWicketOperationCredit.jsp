...
<form name="EditForm" method="POST" action="<c:url value='/main.do'/>?Page=financial/wicket/manageWicketOperationCredit.jsp&ts=<%=getTs()%>">
    ...
    <table class='list' border='0' width='100%' cellspacing='1' cellpadding='0'>
        ...
        <tr>
            ...
            <td class="admin2">
            <% 	if(SH.ci("allowWicketCreditDateChange",1)==1){ %>
            		<%=SH.writeDateField("EditWicketOperationDate","EditForm",sEditWicketOperationDate,true,false,sWebLanguage,sCONTEXTPATH)%>
            <%	}
            	else {	
            %>
            		<input type='text' class='text' size='10' name='EditWicketOperationDate' id='EditWicketOperationDate' value='<%=sEditWicketOperationDate%>' readonly/>
            <%	} 
            	if(version > 1){
