...
<%@ page import="be.openclinic.finance.*,
                 ...
                 java.text.*,be.mxs.common.util.system.*" %>
...
<%
	...
	<form name='EditForm' id="EditForm" method='POST'>
	    <table id='table2' class='list' border='0' width='100%' cellspacing='1' style='border-collapse: collapse;'>
	        <tr>
        ...
        <%	
        	...
        	if(patientInvoice!=null && patientInvoice.getUid()!=null && patientInvoice.getUid().length()>0){
        		...
        		//OBR BEGIN 
        		Vector pointerss =Pointer.getFullPointers("INVSERVSIGN."+patientInvoice.getUid());
        		if(pointerss.size() > 0){
        		signatures+=", Responsable de service: ";
        		}
        		for(int n=0;n<pointerss.size();n++){
        			if(n>0){
        				signatures+=",";
        			}
        			String ptrs=(String)pointerss.elementAt(n);
        			signatures+=ptrs.split(";")[0]+" - "+ScreenHelper.fullDateFormat.format(new SimpleDateFormat("yyyyMMddHHmmSSsss").parse(ptrs.split(";")[1]));
        		}
        		//OBR END
        ...
        <% 
        if(SH.ci("enableOpenIMIS",0)==1 && (patientInvoice==null || !patientInvoice.hasValidUid() || patientInvoice.getDebets().size()==0)){
        %>
            <tr style='border:2px solid black'>
        	<td class='admin'><img style='vertical-align: middle' src='<%=sCONTEXTPATH%>/_img/icons/icon_openimis.png' height='16px'/> OpenIMIS</td>
        	<td class='admin2' id='openIMISMsg' colspan='3'>
        		<table width='100%'>
        <%
        	Insurance openIMISInsurance=null;
        	String openIMISPatientId=null;
    		Vector<Insurance> debetInsurances = Insurance.getCurrentInsurances(activePatient.personid);
        	Iterator<Insurance> i = debetInsurances.iterator();
        	while(i.hasNext()){
        		Insurance debetInsurance = i.next();
        		if(debetInsurance!=null && debetInsurance.getInsurar()!=null && debetInsurance.getInsurar().isOpenIMISConfigured()){
        			openIMISInsurance=debetInsurance;
        			if(SH.c(debetInsurance.getInsuranceNr()).length()>0){
        				openIMISPatientId=debetInsurance.getInsuranceNr();
        			}
        		}
        	}
        	if(openIMISInsurance==null){
        		out.println("<tr><td style='color: red'><img src='"+sCONTEXTPATH+"/_img/icons/icon_warning.gif'/> "+getTran(request,"web","noInsuranceForOpenIMISConfigured",sWebLanguage)+"</td></tr>");
        	}
        	if(openIMISPatientId==null){
        		out.println("<tr><td style='color: red'><img src='"+sCONTEXTPATH+"/_img/icons/icon_warning.gif'/> "+getTran(request,"web","missingOpenIMISPatientId",sWebLanguage)+"</td></tr>");
        	}
        	Encounter encounter = Encounter.getActiveEncounter(activePatient.personid);
        	if(encounter!=null && be.openclinic.medical.Diagnosis.selectDiagnoses("", "", encounter.getUid(), "", "", "", "", "", "", "", "", "", "").size()==0){
        		out.println("<tr><td style='color: red'><img src='"+sCONTEXTPATH+"/_img/icons/icon_warning.gif'/> "+getTran(request,"web","missingOpenIMISDiagnoses",sWebLanguage)+"</td></tr>");
        	}
        	if(SH.cs("OpenIMISProviderCode","").length()==0){
        		out.println("<tr><td style='color: red'><img src='"+sCONTEXTPATH+"/_img/icons/icon_warning.gif'/> "+getTran(request,"web","missingOpenIMISProviderCode",sWebLanguage)+"</td></tr>");
        	}
        	if(SH.c(activeUser.getParameter("organisationid")).length()==0){
        		out.println("<tr><td style='color: red'><img src='"+sCONTEXTPATH+"/_img/icons/icon_warning.gif'/> "+getTran(request,"web","missingOpenIMISEntererCode",sWebLanguage)+"</td></tr>");
        	}
        	%>
        	</table>
        </td>
        ...
        <tr style='border:2px solid black'>
        	...
        	<td class='admin2' id='openIMISMsg' colspan='3'>
        		<table width='100%'>
		        <%
			        String pointer=Pointer.getPointer("sentToOpenIMIS."+patientInvoice.getUid());	
		        	String errorpointer=Pointer.getPointer("sentToOpenIMISWithErrors."+patientInvoice.getUid());
			        if(pointer.length()==0 || errorpointer.length()>0){
			        	boolean bOk=true;
			        	Insurance openIMISInsurance=null;
			        	String openIMISPatientId=null;
		        		HashSet debetInsurances = patientInvoice.getInsurances();
			        	Iterator i = debetInsurances.iterator();
			        	while(i.hasNext()){
			        		Insurance debetInsurance = Insurance.get((String)i.next());
			        		if(debetInsurance!=null && debetInsurance.getInsurar()!=null && debetInsurance.getInsurar().isOpenIMISConfigured()){
			        			openIMISInsurance=debetInsurance;
			        			if(SH.c(debetInsurance.getInsuranceNr()).length()>0){
			        				openIMISPatientId=debetInsurance.getInsuranceNr();
			        	...
			        	if(openIMISInsurance==null){
				        	...
				        	for(int n=0;n<patientInvoice.getDebets().size();n++){
				        		Debet debet = (Debet)patientInvoice.getDebets().elementAt(n);
				        		if(debet.getInsurance()!=null && debet.getInsurance().getUid().equalsIgnoreCase(openIMISInsurance.getUid())){
				        			if(debet.getPrestation()!=null && SH.c(debet.getPrestation().getNomenclature()).length()==0){
						        		out.println("<tr><td style='color: red'><img src='"+sCONTEXTPATH+"/_img/icons/icon_warning.gif'/> "+getTran(request,"web","missingOpenIMISNomenclatureFor",sWebLanguage)+" <b>"+debet.getPrestation().getDescription()+"</b></td></tr>");
						        		bOk=false;
				        			}
				        		}
				        	}
			        	...
			        	if(patientInvoice.getDiagnoses("icd10", 3).size()==0){
			        		out.println("<tr><td style='color: red'><img src='"+sCONTEXTPATH+"/_img/icons/icon_warning.gif'/> "+getTran(request,"web","missingOpenIMISDiagnoses",sWebLanguage)+"</td></tr>");
			        		bOk=false;
			        	}
			        	if(SH.cs("OpenIMISProviderCode","").length()==0){
			        	...
			        	if(SH.c(User.get(Integer.parseInt(patientInvoice.getUpdateUser())).getParameter("organisationid")).length()==0){
			        		out.println("<tr><td style='color: red'><img src='"+sCONTEXTPATH+"/_img/icons/icon_warning.gif'/> "+getTran(request,"web","missingOpenIMISEntererCode",sWebLanguage)+"</td></tr>");
			        		bOk=false;
			        	}
			        	...
			        	if(bOk){
			        		out.println("<tr><td><input type='button' class='button' name='validateOpenIMISButton' onclick='validateOpenIMIS()' value='"+getTranNoLink("web","validateOpenIMIS",sWebLanguage)+"'/></tr>");
			        	}
		        	}
		        	if(pointer.length()>0){
		        		out.println("<tr><td>"+getTran(request,"web","sent",sWebLanguage)+": "+pointer+(errorpointer.length()>0?" <img src='"+sCONTEXTPATH+"/_img/icons/icon_warning.gif' height='14px'/> <b>"+getTran(request,"web","invoicesentwitherrors",sWebLanguage):"")+"</b></td></tr>");
		        	}
                        ...
                        <%
                            	...
                            	if((MedwanQuery.getInstance().getConfigInt("authorizeCancellationOfOpenInvoices",1)==1 && patientInvoice.getStatus().equalsIgnoreCase("closed"))||(activeUser.getParameter("sa")!=null && activeUser.getParameter("sa").length() > 0)||activeUser.getAccessRight("financial.cancelclosedinvoice.select")){
                            		%>
                            		 ...
                            		 <% //*** OBR BEGIN ******************%>
                                	<input class="button" type="button" name="buttonNoteDeCredit" value='<%=getTranNoLink("Web.finance","notedecredit",sWebLanguage)%>' onclick="doNoteDeCredit('<%=patientInvoice.getUid()%>');">
                                	<input class="button" type="button" name="buttonServiceValidation" value='<%=getTranNoLink("Web.finance","Signature resp",sWebLanguage)%>' onclick="doServiceSign('<%=patientInvoice.getUid()%>');">
                                     <% //*** OBR END ******************%>
	    ...
	    <%
	    java.util.Date annulation = OBR.getCancel(patientInvoice.getUid());
	    String annulation_status = "";
	    if(annulation!=null){
	    	annulation_status = "<span style='color:#fff;'>Cancelled: "+annulation+"<span>";	
	    }
	    %>
	    
	    <% 
	  //*** OBR BEGIN ******
	    if(patientInvoice!=null && patientInvoice.hasValidUid() && MedwanQuery.getInstance().getConfigInt("enableMedHub",0)==1){
	    	if(activeUser.getAccessRight("financial.modifyinvoice.select")){ %>
			    <table width="100%" class="medhub"><tr class="medhub" >
			    
			    <td class="medhub" style="width:25%">MEDHUB:<%=MedHub.getSendingStatus(patientInvoice.getUid(),sCONTEXTPATH) %> <a href="#" onclick="resendInvoice('<%=patientInvoice.getUid()%>','MedHub');"><%=getTranNoLink("Web.finance","resend",sWebLanguage)%></a><span id='result_resend_medhub' name='result_resend_medhub'></span></td>
			    <td class="medhub" style="width:25%"><%=MedHub.getErrorOnIsu(patientInvoice.getUid(),sCONTEXTPATH) %></td>
			    <td class="medhub" style="width:25%"><%=Medhubmessage.countInvoiceMessages(patientInvoice.getUid(),sCONTEXTPATH) %></td>
			    <td class="medhub" style="width:25%" ></td>
			    
			    </tr></table >
		<% 	}
	    }
		if(patientInvoice!=null && patientInvoice.hasValidUid() && MedwanQuery.getInstance().getConfigInt("enableOBR",0)==1){ 
			if(activeUser.getAccessRight("financial.modifyinvoice.select")){
			    %>
			    <table style="width:100%" class="medhub" >
				    <tr>  
					    <td style="width:10%" ><img height='50px' src='<%=sCONTEXTPATH%>/_img/obr.png'/></td> 
					    <td style="width:15%" >OBR:<%=OBR.getSendingStatus(patientInvoice.getUid(),sCONTEXTPATH) %> <a href="#" onclick="resendInvoice('<%=patientInvoice.getUid()%>','OBR');"><%=getTran(request,"web.finance","resend",sWebLanguage)%></a>  <span id='result_resend_obr' name='result_resend_obr'></span></td> 
					    <td class="medhub" style="width:25%" >&nbsp;&nbsp;&nbsp;<span id='errmsg'><%=OBR.getErrorOfDuplication(patientInvoice.getUid(),sCONTEXTPATH) %></span></td>
					    <td class="medhub" style="width:10%" ><center><%=annulation_status %><a href='#' onclick='OpenDiscussion(<%=patientInvoice.getUid()%>)'><br/><img style='vertical-align: middle' src='<%=sCONTEXTPATH %>/_img/icons/icon_incoming.gif' /></a></center></td>
					    <td style="width:10%" ><center><img src='<%=QrCodeUtil.toBase64QrCode(OBR.getSignature(patientInvoice.getUid()),75,75)%>'/></center></td>
					    <td style="width:30%" ><%= OBR.getSignature(patientInvoice.getUid()).replaceAll("/", " / ") %></td>
				    </tr>
				</table>
	  <% 	}
		} 
	   ...
	   <script>
		...
		//*** OBR BEGIN ******
	              function doServiceSign(invoiceuid){
	      	        var today = new Date();
	      	        var url= '<c:url value="/financial/medhub/ServicePatientInvoiceSign.jsp"/>?ts='+today;
	      	        new Ajax.Request(url,{
	      	              method: "POST",
	      	              postBody: 'EditInvoiceUID=' + invoiceuid,
	      	              onSuccess: function(resp){
	    ...
	    function validateOpenIMIS(){
	        openPopup("/financial/validateOpenIMIS.jsp&ts=<%=getTs()%>&invoiceuid="+document.getElementById("EditInvoiceUID").value+"&PopupWidth=600&PopupHeight=400");
	    }
	    function sendToOpenIMIS(){
	       openPopup("/financial/sendInvoiceToOpenIMIS.jsp&ts=<%=getTs()%>&invoiceuid="+document.getElementById("EditInvoiceUID").value+"&status=claim&PopupWidth=600&PopupHeight=400");
	    }
	    ...
	    function doModifyInvoice(invoiceuid){
  		...
  		//*** OBR BEGIN ******
  		  ...
  		  function doNoteDeCredit(invoiceuid){
  			  
  		    var params = '';
		    var today = new Date();
		    let motif;
		    let person = prompt("Veillez fournir la raison d'annulation ", "Facture non conforme");
		    if(person != null || person != ""){
		    	motif = person;
		    	//alert(motif);
		    var url= '<c:url value="/financial/medhub/creditNoteInvoice.jsp"/>?motif='+motif+'&invoiceuid='+invoiceuid+'&ts='+today;
		        //alert(url);
		    document.getElementById('patientInvoiceDebets').innerHTML = "<img src='<c:url value="/_img/themes/default/ajax-loader.gif"/>'/><br/>Loading";
		    new Ajax.Request(url,{
			  method: "GET",
		      ...
		      onSuccess: function(resp){
		       	...
		       	if(label.invoiceuid.length>0){   
		          if(label.invoiceuid){
		        	  alert("L'OBR confirme l'annulation de la facture dans son systeme");
		        	  setPatientInvoice(invoiceuid);
		          }
		        };
		      },
			  onFailure: function(request, status, error){
		...
		//*** OBR BEGIN ******
	      function resendInvoice(invoiceUid,module){
		
		  if(module.localeCompare("OBR")==0){
              $('result_resend_obr').innerHTML="Sending....";
        	  }
              
              if(module.localeCompare("MedHub")==0){     
               $('result_resend_medhub').innerHTML="Sending....";
              }
		
		
        var today = new Date();
        var url= '<c:url value="/financial/medhub/resend.jsp"/>?ts='+today;
        new Ajax.Request(url,{
              method: "POST",
              postBody: 'EditPatientInvoiceUID=' + invoiceUid +
                        '&module=' + module,
    			        
              onSuccess: function(resp){
            	  
            	  var label = eval('('+resp.responseText+')');
                  var resultat = label.resultat;
                  
                  if(module.localeCompare("OBR")==0){
                    	$('result_resend_obr').innerHTML= resultat;
                      	$('errmsg').innerHTML= resultat;
            	  }
                  
                  if(module.localeCompare("MedHub")==0){
                   	$('result_resend_medhub').innerHTML= resultat;
                  }
              ...
              onFailure: function(){
                  //$('divMessage').innerHTML = "Error in function markCorrected() => AJAX";
            	   ...
            	   if(module.localeCompare("OBR")==0){
                       $('result_resend_obr').innerHTML="Erreur";
                 	  }
                       
                   if(module.localeCompare("MedHub")==0){
                        $('result_resend_medhub').innerHTML="Erreur";
                    }
              }
          }
        );
