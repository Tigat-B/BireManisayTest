...
%>
 ...
 %>
        ...
        <%=ScreenHelper.setFormButtonsStart()%>
            ...
            <input class="button" type="button" name="CheckOpenIMISButton" id="CheckOpenIMISButton" value='<%=getTranNoLink("Web","validate",sWebLanguage)%>' onclick="checkOpenIMIS();">
...
<script>
  ...
  function checkOpenIMIS(){
	openPopup("/financial/checkOpenIMISCoverage.jsp&ts=<%=getTs()%>&PopupWidth=800&PopupHeight=400&insurancenr="+EditInsuranceForm.EditInsuranceNr.value+"&insuranceuid=<%=sEditInsuranceUID%>&insurarid="+EditInsuranceForm.EditInsurarUID.value);
  }
  ...
  function checkInsuranceAuthorization(force,ignorewarnings){
    ...
    new Ajax.Request(url,{
      ...
      onSuccess: function(resp){
        ...
        if($('openIMISConfigured') && $('openIMISConfigured').value=='1'){
	    	$('CheckOpenIMISButton').style.display='';
    	}
    	else{
	    	$('CheckOpenIMISButton').style.display='none';
    	}
      },
	  onFailure: function(){
	    alert('error');
