    ...
    <%	
    	if(invoice!=null && invoice.hasValidUid()){
    		...
    		for(int n=0;n<pointers.size();n++){
    		 ...
    		 //--OBR BEGIN--
    		String signaturesresp="";
    		Vector pointersreps=Pointer.getFullPointers("SUMRESPINVSIGN."+invoice.getUid());
    		if(pointersreps.size() > 0){
    			 signaturesresp=", Resp: ";	
    		}
    		for(int n=0;n<pointersreps.size();n++){
    			if(n>0){
    				signaturesresp+=", ";
    			}
    			String ptr=(String)pointersreps.elementAt(n);
    			signaturesresp+=ptr.split(";")[0]+" - "+ScreenHelper.fullDateFormat.format(new SimpleDateFormat("yyyyMMddHHmmSSsss").parse(ptr.split(";")[1]));
    		}
    		
    		//--OBR END--
    		
    		if(signatures.length()>0 || signaturesresp.length()>0 ){
 	        %>
 	        <tr>
...
<%
	...
	<%
		...
		<input class="button" type="button" name="buttonRespSignature" value='<%=getTranNoLink("Web.finance","Responsable",sWebLanguage)%>' onclick="doSignResp('<%=invoice.getUid()%>');">
