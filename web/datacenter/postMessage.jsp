	...
	if(type.equalsIgnoreCase("post")){
        ...
        try{
			...
			while(msgs.hasNext()){
		    	...
		    	if(key.length()>0 && SH.ci("autoLearnRemoteServers",0)==1){
			    	...
			    	if(!rs.next()){
			    		rs.close();
			    		ps.close();
			    		ps = conn.prepareStatement("insert into dc_servers(dc_server_serverid,dc_server_key) values(?,?)");
				    	ps.setInt(1,serverid);
				    	ps.setString(2,key);
				    	ps.execute();
				    	ps.close();
				    	SH.syslog("Added dc_server "+serverid+" with key "+key);
			    	}
			    	rs.close();
			    	ps.close();
		    	}
