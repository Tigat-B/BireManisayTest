					...
					if(mrequest.getParameter("filetype").equalsIgnoreCase("prestationscsv")){
						...
						while(reader.ready()){
							...
							if(line.length<3){
								...
								service.store();
								MedwanQuery.getInstance().storeLabelWithDelete("service", code, "fr", name, Integer.parseInt(activeUser.userid));
								MedwanQuery.getInstance().storeLabelWithDelete("service", code, "en", name, Integer.parseInt(activeUser.userid));
							}
						}
						reader.close();
						reloadSingleton(session);
