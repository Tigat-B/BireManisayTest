...
<%
    ...
    if(sOut.length() > 0){
        %>
        <table width="100%" align="center" cellspacing="1px" cellpadding='5px' class="list">
          <tr class="admin">
              <td width="15%">Key</td>
              <td width="70%"><%=HTMLEntities.htmlentities(getTran(request,"Web","value",sWebLanguage))%></td>
              <td ><%=HTMLEntities.htmlentities(getTran(request,"Web","updatetime",sWebLanguage))%></td>
          </tr>
          <tbody class="hand">
              <%=sOut%>
          </tbody>
