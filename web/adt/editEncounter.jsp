...
<%
    ...
    if(sEditEncounterUID.trim().length()==0 && (activePatient.isDead()!=null && !activeUser.getAccessRightNoSA("deceasedpatient.add"))){
	    ...
	    String sEditEncounterResponsibleService = checkString(request.getParameter("EditEncounterResponsibleService"));
	    String sEditEncounterResponsibleServiceName = checkString(request.getParameter("EditEncounterResponsibleServiceName"));
	    String sEditEncounterCondition = checkString(request.getParameter("EditEncounterCondition"));
	    ...
	    if(Debug.enabled){
	        ...
	        Debug.println("EncounterResponsibleService         : "+sEditEncounterResponsibleService);
	        Debug.println("EncounterResponsibleServiceName     : "+sEditEncounterResponsibleServiceName);
	        Debug.println("EncounterCondition     	: "+sEditEncounterCondition);
	    ...
	    if(sAction.equals("SAVE")){
			...
			tmpEncounter.setCondition(sEditEncounterCondition);
	        ...
	        tmpEncounter.setResponsibleService(sEditEncounterResponsibleService);
	    ...
	    if(sEditEncounterUID.length() > 0){
			...
			sEditEncounterCondition =SH.c(tmpEncounter.getCondition());
        	...
        	sEditEncounterResponsibleService=tmpEncounter.getResponsibleService();
        	if(sEditEncounterResponsibleService.length()>0){
        		sEditEncounterResponsibleServiceName = checkString(getTran(request,"service",tmpEncounter.getResponsibleService(),sWebLanguage));
        	}
	...
	%>
	    ...
	    <table class='list' border='0' width='100%' cellspacing='1'>
	        ...
	        <tr>
	            ...
	            <td class="admin2">
	                <select class="text" name="EditEncounterOrigin">
	                    <option/>
	                    <%=ScreenHelper.writeSelect(request,SH.cs("encounterOrigin","urgency.origin"),sEditEncounterOrigin,sWebLanguage)%>
	                </select>
	            </td>
	        </tr>
	        ...
	        <% if(SH.ci("enableResponsibleServiceInEncounter",0)==1){ %>
		        <%-- RESPONSIBLE service --%>
		        <tr id="ResponsibleService">
		            <td class="admin"><%=getTran(request,"web","responsibleservice",sWebLanguage)%></td>
		            <td class='admin2'>
		                <input type="hidden" name="EditEncounterResponsibleService" id="EditEncounterResponsibleService" value="<%=sEditEncounterResponsibleService%>">
		                <input class="text" type="text" name="EditEncounterResponsibleServiceName" id="EditEncounterResoponsibleServiceName" readonly size="<%=sTextWidth%>" value="<%=sEditEncounterResponsibleServiceName%>" >
		                
		                <img src="<c:url value="/_img/icons/icon_search.png"/>" class="link" alt="<%=getTran(null,"web","select",sWebLanguage)%>" onclick="searchService('EditEncounterResponsibleService','EditEncounterResponsibleServiceName',true);">
		                <img src="<c:url value="/_img/icons/icon_delete.png"/>" class="link" alt="<%=getTran(null,"web","clear",sWebLanguage)%>" onclick="EditEncounterForm.EditEncounterResponsibleService.value='';EditEncounterForm.EditEncounterResponsibleServiceName.value='';">
		            </td>
		        </tr>
	        <% } %>
	        <% if(SH.ci("enableConditionInEncounter",0)==1){ %>
		        <%-- RESPONSIBLE service --%>
		        <tr id="Condition">
		            <td class="admin"><%=getTran(request,"web","condition",sWebLanguage)%></td>
		            <td class='admin2'>
						<input type='text' class='text' size='<%=sTextWidth%>' id='EditEncounterCondition' name='EditEncounterCondition' value='<%=sEditEncounterCondition %>'/>
		            </td>
		        </tr>
	        <% } %>
	        <%-- manager --%>
	       ...
	       <tr id="Bed" style="visibility:visible;">
	        ...
	        <tr>
	            ...
	            <td class="admin2">
	                <select class="text" name="EditEncounterSituation" id="EditEncounterSituation" style="vertical-align:top;">
	                	...
	                	%>
	                    <%=ScreenHelper.writeSelect(request,SH.cs("encounterSituation","encounter.situation"),sEditEncounterSituation,sWebLanguage)%>
	                </select>
	            </td>
	        </tr>
	        ...
	        <tr>
	            ...
	            <td class="admin2">
	                <select class="text" name="EditEncounterOutcome" style="vertical-align:middle;">
	                    ...
	                    <%=ScreenHelper.writeSelectUnsorted(request,SH.cs("encounterOutcomeType","encounter.outcome"),sEditEncounterOutcome,sWebLanguage)%>
	                </select>
			        <%
	        ...
	        <%
	            ...
	            if(tmpEncounter!=null && tmpEncounter.hasValidUid()){
	                %><input class='button' type="button" id="timeLineButton" name="timeLineButton" value='<%=getTranNoLink("web","timeline",sWebLanguage)%>' onclick="doTimeLine();">&nbsp;<%
	            }
	...
	<script>  
	  ...
	  function doTimeLine(){
		    window.open("<c:url value='/util/csvStats.jsp?'/>query=encounter.timeline&encounterUid=<%=tmpEncounter==null?"":tmpEncounter.getUid()%>");
	  }
	  ...
	  function searchService(serviceUidField,serviceNameField,nocheck){
	    ...
	    if(serviceNameField!='EditEncounterDestinationName'){
		  if(document.getElementById("EditEncounterType").value=='admission' && !nocheck){
