<%@page import="org.apache.poi.hssf.record.UserSViewBegin"%>
...
%>
	<form name='transactionForm' id='transactionForm' method='POST'>
		...
		<br/>
		<center>
			<input type='button' onclick='doPrint();' name='printButton' class='button' value='<%=getTranNoLink("web","printtodaysappointments",sWebLanguage) %>'/>
		</center>
	</form>
<%
	}
...
<script>
    ...
    function doPrint(){
    	var isPatient='<%=calendarType%>'=='patient';
    	var day = document.getElementById('begin').value.substring(0,2);
    	var month = document.getElementById('begin').value.substring(3,5);
    	var year = document.getElementById('begin').value.substring(6,10);
    	<%
    		if(begindate!=null){
	    		java.util.Date date = new SimpleDateFormat("yyyy-MM-dd").parse(begindate.split("T")[0]);
	    		String dow="2345671".substring(Integer.parseInt(new SimpleDateFormat("u").format(date))-1, Integer.parseInt(new SimpleDateFormat("u").format(date)));
	    	%>
		    window.open("<c:url value='/planning/ajax/getCalendarPdf.jsp'/>?ts="+new Date().getTime()+"&dayToShow=<%=dow%>&year="+year+"&month="+month+"&day="+day+"&ispatient="+isPatient+"&PatientID=<%=activePatient.personid%>&FindUserUID=<%=session.getAttribute("calendarUser")%>");
		<% }%>
    }
