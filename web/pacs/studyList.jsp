...
<%
	...
	<%
		...
		if(request.getParameter("loadMPI")==null && MedwanQuery.getInstance().getConfigInt("enableMPI",0)==1){
			if(SH.c(activePatient.adminextends.get("mpiid")).length()>0){
	%>
				<input type='submit' class='button' name='loadMPI' value='<%=getTranNoLink("web","loadMPIimages",sWebLanguage) %>'/>
	<%
			}
	...
	%>
				<input type='button' onclick='alert("<%=getTranNoLink("web","registermpiidfirst",sWebLanguage) %>")' class='redbutton' name='loadMPI' value='<%=getTranNoLink("web","loadMPIimages",sWebLanguage) %>'/>
	<%
			}
		}
	}
