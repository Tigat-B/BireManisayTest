...
<%
	...
	if(MedwanQuery.getInstance().getConfigInt("enableMPI",0)==1){
		...
		if(sMPIID.length()>0){
			sMPI=" | <a href='javascript:showMPI()'><font style='font-size:12px'>"+SH.cs("mpiServerName","KHIN")+": "+sMPIID+"</font></a>";
		}
		else{
			sMPI=" | <a href='javascript:showMPI()'><font style='font-size:12px'>"+SH.cs("mpiServerName","KHIN")+": </font><img height='14px' style='vertical-align: middle' src='"+sCONTEXTPATH+"/_img/icons/icon_erase.gif'/></a>";
		}
	}
	
	String sOpenIMIS="";
	if(SH.ci("enableOpenIMIS",0)==1 && SH.c(activePatient.getID("natreg")).length()>0){
		sOpenIMIS="<span id='openimisloader'>&nbsp;&nbsp;&nbsp;<img onclick='importOpenIMISActiveInsuree();' style='vertical-align: middle' height='20px' src='"+sCONTEXTPATH+"/_img/icons/icon_openimis.png'/></span>";
	}
%>
<script>
...
<table width="100%" class="list" cellpadding="1" cellspacing="0">
    <tr>
        <td colspan="10" class="titleadmin">
            <div style="float:left;vertical-align:middle"><%=getTran(request,"web","administrative.data",sWebLanguage)+" "+sMPI+" "+sVip+" "+sDiscontinued+" "+sOpenIMIS%></div>
