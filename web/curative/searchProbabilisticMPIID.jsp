...
<table width='100%'>
	...
	if(sortedPatients.size()>0){
		%>
		<tr>
			<td colspan='5'><input type='button' class='button' name='newmpibutton' onclick='creatempi()' value='<%=getTranNoLink("web","createnewmpiid",sWebLanguage)%>'/></td>
		</tr>
		<%
		...
		while(iSortedPatients.hasNext()){
			...
			<tr style='vertical-align: top'>
				...
				<td class='admin2' style='vertical-align: top'><%=checkString(p.getActivePrivate().district)+", "+checkString(p.getActivePrivate().city)+", "+checkString(p.getActivePrivate().address) %></td>
