...
<%
	...
	ResultSet rs = null;
	if(productstock!=null && activePatient!=null){
			...
			if(!servicestock.equalsIgnoreCase("-1")){
				...
				if(quantity!=0){
					...
					for(int n=0; n<batches.size(); n++){
						...
						if(activePatient!=null && productstock!=null && batch!=null){
							ps = conn.prepareStatement("SELECT sum(OC_LIST_QUANTITY) reserved FROM OC_DRUGSOUTLIST WHERE OC_LIST_PATIENTUID=?"+
		...
		while(rs.next()){
			...
			if(stock!=null && stock.getProduct()!=null){
				...
				int nBatch=rs.getInt("OC_LIST_QUANTITY");
				 ...
				 String fontstyle=stock.getProduct().getPrestationcode()==null || stock.getProduct().getPrestationcode().length()==0 || !stock.getProduct().isAutomaticInvoicing()?"color: white;background-color: red;font-size: 8;font-weight: bolder;text-decoration: line-through":"color: #606060;font-size: 8";
				 drugs+= "<tr>"+
				         "<td class='admin2'>"+
