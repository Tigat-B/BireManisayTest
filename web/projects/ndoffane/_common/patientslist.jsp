...
<%
    ...
    if(lResults.size() > 0){
        ...
        while((iOverallCounter+iCounter) < lResults.size() && iCounter < iMaxResultSet){
            ...
            sCity="";
        	String[] privateDetails= new String[21];
            if(bShowCity || activeUser.getParameter("patientlist", "extended").equalsIgnoreCase("extended")){
            ...
            sResult.append(" class='list"+sInactive+sClass+"'>"
                   ...
                   +""+sTmpServiceID
                   +"</tr>");
            if(activeUser.getParameter("patientlist", "extended").equalsIgnoreCase("extended")){
            	//Add an extra line of patient information
				String post="",village="",district="",address=SH.c(privateDetails[3]),ins="",phone="";
				if(SH.c(privateDetails[18]).split(";").length>1){
					district=privateDetails[18].split(";")[1];
				}
				if(SH.c(privateDetails[18]).split(";").length>2){
					post=privateDetails[18].split(";")[2];
				}
				if(SH.c(privateDetails[18]).split(";").length>3){
					village=privateDetails[18].split(";")[3];
				}
				village+=" "+SH.c(privateDetails[19]);
                ...
                sResult.append("<tr onClick='window.location.href=\""+sLink+"\";' class='list"+sInactive+sClass+"small'><td colspan='3'/><td>"+getTran(request,"web","address",sWebLanguage)+": <font style='"+sSmallFontStyle+"'>"+address+"</font></td><td colspan='2'>"+getTran(request,"web","district",sWebLanguage)+": <font style='"+sSmallFontStyle+"'>"+district+"</font></td><td>"+getTran(request,"web","post",sWebLanguage)+": <font style='"+sSmallFontStyle+"'>"+post+"</font></td><td colspan='2'>"+getTran(request,"web","city",sWebLanguage)+": <font style='"+sSmallFontStyle+"'>"+village+"</font></td><td><font style='"+sSmallFontStyle+"'>"+ins+"</font></td></tr>");
            }
            iCounter++;
        }
