...
<%!
	...
	private String printMonitor(String personid,String deviceid,java.sql.Timestamp ts,double baseFontSize,double cellHeight){
		...
		SortedSet<java.sql.Timestamp> movements = new TreeSet<java.sql.Timestamp>();
		...
		try{
			...
			while(rs.next()){
				movements.add(rs.getTimestamp("ts"));
			}
			rs.close();
			ps.close();
			if(movements.size()>0){
			...
			if(SH.c(personid).length()>0){
				...
				ps.setString(1,personid);
				ps.setTimestamp(2,new java.sql.Timestamp(ts.getTime()-SH.getTimeSecond()*5));
			}
			else{
			...
			while(rs.next() && results.size()<5){
				if(movements.contains(rs.getTimestamp("ts"))){
					continue;
				}
			...
			if(SH.cs("deviceLocation."+deviceid,"").equals("thorax")){
				sHeartRate="HR: "+getResult("9279-1",results,resultsCounter)+" bpm";
				dHeartRate=getNumericResult("9279-1",results,resultsCounter);
				sRespiratoryRate="Resp: "+getResult("8867-4",results,resultsCounter)+" rpm";
				dRespiratoryRate=getNumericResult("8867-4",results,resultsCounter);
			}
