...
<%@page import="org.dom4j.*,org.dom4j.tree.*"%><%@page import="be.openclinic.system.Encryption"%><%@page errorPage="/includes/error.jsp"%><%@include file="/includes/helper.jsp"%><%
	...
	if(rs.next()){
		...
		if(checkString(mrequest.getParameter("ghb_ref_serverid")).length()>0){
			...
			if(rs.next()){
				...
				if(sToken.equals(Encryption.decryptTextWithPublicKey(sCrypToken, sPk))){
					if(checkString(mrequest.getParameter("delete")).equalsIgnoreCase("1")){
						...
						ps=conn.prepareStatement("select * from GHB_SERVERS where GHB_SERVER_DOMAIN=? and GHB_SERVER_ID<>?");
						ps.setString(1,sDomain);
						ps.setInt(2,nServerId);
						rs=ps.executeQuery();
						if(rs.next()){
							root.setAttributeValue("error", "E3: Server domain "+sDomain+" already exists. It must be unique");
							rs.close();
							ps.close();
						}
						else{
			...
			ps=conn.prepareStatement("select * from GHB_SERVERS where GHB_SERVER_DOMAIN=?");
			ps.setString(1,sDomain);
			rs=ps.executeQuery();
			if(rs.next()){
				root.setAttributeValue("error", "E3: Server domain "+sDomain+" already exists. It must be unique");
