	...
	for(int n=0;n<documents.size();n++){
		...
		if(document.deleteDate==null){
		    ...
		    if(SH.c(document.storageName).length()>0 && file.exists()){
		    	out.println("<td class='admin2'><a href='javascript:getFile(\"/"+document.storageName+"\")'><img title='"+getTranNoLink("web","download",sWebLanguage)+"' height='16px' src='"+sCONTEXTPATH+"/_img/icons/mobile/download.png'/></a> <b>"+document.title+"</b></td>");
		    }
		    else {
		    	out.println("<td class='admin2'><img title='"+getTranNoLink("web","unavailable",sWebLanguage)+"' height='16px' src='"+sCONTEXTPATH+"/_img/icons/mobile/nosave.png'/> <b>"+document.title+"</b></td>");
		    }
			out.println("</tr>");
		}
	}
