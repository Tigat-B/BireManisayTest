...
<script>    
  ...
  function getCPUTemperature(id){
      var url = '<c:url value="/"/>/system/ajax/getTemperature.jsp?ts=<%=getTs()%>';
      new Ajax.Request(url,{
        method: "GET",
        parameters: "",
        onSuccess: function(resp){
            var label = eval('('+resp.responseText+')');
            document.getElementById(id).innerHTML=label.cpu+"°C";
            if(label.cpu*1<<%=SH.ci("tempWarning",50)%>){
            	document.getElementById(id).style.color='green';
            	document.getElementById(id).style.fontSize='10px';
            }
            else if(label.cpu*1<<%=SH.ci("tempCritical",70)%>){
        	...
        	window.setTimeout("getCPUTemperature('temperature');",10000);
        }
      });
  }
  ...
  function translateItems(transactionType){
	    openPopup("/system/translateItems.jsp&transactiontype="+transactionType+"&ts=<%=getTs()%>");
	  }
  ...
  function updateMultiCheckbox(cbField,hiddenField,sID){
	  window.setTimeout("updateMultiCheckbox2('"+cbField.id+"','"+hiddenField+"','"+sID+"')",200);
  }
  function updateMultiCountbox(cbField,hiddenField,sID){
	  if(cbField.value*1>0){
		  existingFields= document.getElementById(hiddenField).value.split(";");
		  for(n=0;n<existingFields.length;n++){
			 field=existingFields[n];
			 if(field.split("=")[0]==sID){
				 document.getElementById(hiddenField).value=document.getElementById(hiddenField).value.replace(field+";",sID+"="+cbField.value+";");
				 return;
			 }
		  }
		  document.getElementById(hiddenField).value=document.getElementById(hiddenField).value+sID+"="+cbField.value+";";
	  }
	  else{
		  existingFields= document.getElementById(hiddenField).value.split(";");
		  for(n=0;n<existingFields.length;n++){
			 field=existingFields[n];
			 if(field.split("=")[0]==sID){
				 document.getElementById(hiddenField).value=document.getElementById(hiddenField).value.replace(field+";","");
			 }
		  }
	  }
  }
  
  function updateMultiCheckbox2(cbFieldID,hiddenField,sID){
	  var cbFieldChecked=document.getElementById(cbFieldID).checked;
	  var cbValues = document.getElementById(hiddenField).value;
	  if(cbFieldChecked && cbValues.indexOf(sID+";")<0){
		  document.getElementById(hiddenField).value=document.getElementById(hiddenField).value+sID+";"
	  }
	  else if(!cbFieldChecked){
  ...
  function setAge2(){
		...
		if(age>0){
			<% if(MedwanQuery.getInstance().getConfigInt("approximateDateStartsOnFirstOfJanuary",1)==1){%>
				document.getElementById('DateOfBirth').value='01/01/'+(new Date().getFullYear()-age).toFixed(0);
			<%}else{%>
			var day = new Date().getDate();
			var month = new Date().getMonth()+1;
			if((day+"").length<2){day="0"+day}
			if((month+"").length<2){month="0"+month}
			document.getElementById('DateOfBirth').value=day+'/'+month+'/'+(new Date().getFullYear()-age);
			<%}%>
		}
	}
  ...
  function addAdminArchiveDocument(){
	  window.location.href="/openclinic/healthrecord/createTransaction.do?be.mxs.healthrecord.createTransaction.transactionType=be.mxs.common.model.vo.healthrecord.IConstants.TRANSACTION_TYPE_ARCHIVE_ADMINDOCUMENT";
  }
  ...
  function importOpenIMISActiveInsuree(){
		document.getElementById('openimisloader').style.display='';
		document.getElementById('openimisloader').innerHTML = "&nbsp;&nbsp;&nbsp;<img style='vertical-align: middle' height='10px' src='<c:url value="/_img/themes/default/ajax-loader.gif"/>'/>";
	    var params = "natreg=<%=SH.c(activePatient.getID("natreg"))%>";
  ...
  function showLibrary(){
	  openPopup("util/library.jsp&PopupWidth=1024&PopupHeight=600");
  }
  ...
  function checkMandatoryFields(){
	  ...
	  <%	}%>
	  <%	if(SH.ci("enforceCompleteClinicalDataEntry",1)==1){ %>
	...
	<%if(SH.ci("enableTemperatureMonitoring",0)==1){%>
		window.setTimeout("getCPUTemperature('temperature');",500);
	<%}%>
</script>