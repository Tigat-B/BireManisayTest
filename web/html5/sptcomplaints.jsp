...
<html>
	<body>
		...
		if(activePatient!=null){
			...
			else if(activePatient.getAgeInMonths()<150){
				patientSigns.put("ageinmonths",120);
				age=120;
			}
			else if(activePatient.getAgeInMonths()<180){
				patientSigns.put("ageinmonths",150);
				age=150;
			}
			else if(activePatient.getAgeInMonths()<480){
				patientSigns.put("ageinmonths",240);
				age=240;
			}
			else if(activePatient.getAgeInMonths()<600){
				patientSigns.put("ageinmonths",500);
				age=500;
			}
			else {
				patientSigns.put("ageinmonths",600);
				age=500;
			}
		...
		<%
			...
			out.println("<tr><td class='mobileadmin' colspan='"+(cols)+"'><span style='font-size:4vw;'>"+getTran(request,"web","age",sWebLanguage)+": </span><select style='font-size:6vw;' name='age' onchange='document.getElementById(\"sptaction\").value=\"patient\";transactionForm.submit();'><option style='font-size:6vw;'/><option style='font-size:6vw;' value='1' "+(age==1?"selected":"")+"><2m</option><option style='font-size:6vw;' value='6' "+(age==6?"selected":"")+">2-12m</option><option style='font-size:6vw;' value='24' "+(age==24?"selected":"")+">12m-60m</option><option style='font-size:6vw;' value='120' "+(age==120?"selected":"")+">5-12y</option><option style='font-size:6vw;' value='150' "+(age==150?"selected":"")+">12-15y</option><option style='font-size:6vw;' value='240' "+(age==240?"selected":"")+">15-25y</option><option style='font-size:6vw;' value='480' "+(age==480?"selected":"")+">26-45y</option><option style='font-size:6vw;' value='600' "+(age==600?"selected":"")+">>45y</option></select></td>");
			...
			if(complaints!=null && age>0 && gender.length()>0){
				...
				while(sections.hasNext()){
					...
					concepts = section.elementIterator("concept");
