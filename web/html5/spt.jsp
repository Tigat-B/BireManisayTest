...
<%!
	...
	boolean hasTreatments(Element pathway, Hashtable signs, String sWebLanguage){
		...
		while(childNodes.hasNext()){
			...
			for(int i=0;i<paths.size();i++){
				if(((String)paths.elementAt(i)).split("\\|").length==0){
					continue;
				}
			...
			if(iPaths.hasNext()){
				...
				SH.syslog("title="+title);
				//Skip a path with a treatment that has the attribute hidepath=1
				boolean bSkip=false;
				Iterator qPaths = hPaths.keySet().iterator();
				while(qPaths.hasNext()){
					String qTitle = (String)qPaths.next();
					SH.syslog(qTitle);
					if(!hasLaterNode(hPaths, qTitle)){
						String lastpart=qTitle.split(">")[qTitle.split(">").length-1];
	...
	boolean mustHide(Element pathway, Hashtable signs, Hashtable treatments, String sWebLanguage){
		Iterator childNodes = pathway.elementIterator("node");
		while(childNodes.hasNext()){
			Element childNode = (Element)childNodes.next();
			SortedMap hPaths = new TreeMap();
				...
				while(qPaths.hasNext()){
					...
					if(!hasLaterNode(hPaths, qTitle)){
						...
						if(lastpart.split("\\$").length>1 && treatments.get(lastpart.split("\\$")[1])!=null){
							...
							SH.syslog("A");
							SH.syslog("treatment="+treatment.hide);
							if(treatment!=null && treatment.hide.equalsIgnoreCase("1")){
								SH.syslog("musthide!!!!!");							
								return true;
							}
						}
...
<html>
	<body>
		...
		<%
			...
			if(SH.c(request.getParameter("reset")).equalsIgnoreCase("1")){
			...
			for(int n=0;n<pathwayFiles.length;n++){
				...
				if(root.element("documents")!=null){
					int ageinmonths = (Integer)((Hashtable)signs.get("patient")).get("ageinmonths");
				...
				if(root.element("treatments")!=null){
					...
					while(iTreatments.hasNext()){
						...
						t.hide=SH.c(treatment.attributeValue("hidepath"));
						...
						t.minage = SH.c(treatment.attributeValue("minage"));
						if(t.minage.length()>0 && Integer.parseInt(t.minage)>ageinmonths){
							continue;
						}
						t.maxage = SH.c(treatment.attributeValue("maxage"));
						if(t.maxage.length()>0 && Integer.parseInt(t.maxage)<ageinmonths){
							continue;
						}
						...
						while(iSheets.hasNext()){
							Element sheet = (Element)iSheets.next();							
							if(SH.c(sheet.attributeValue("minage")).length()>0 && Integer.parseInt(SH.c(sheet.attributeValue("minage")))>ageinmonths){
								continue;
							}
							if(SH.c(sheet.attributeValue("maxage")).length()>0 && Integer.parseInt(SH.c(sheet.attributeValue("maxage")))<ageinmonths){
								continue;
							}
							t.sheets.add(checkString(sheet.attributeValue("id")));
						}
						t.activates=new Vector();
						iSheets = treatment.elementIterator("activate");
						while(iSheets.hasNext()){
							Element sheet = (Element)iSheets.next();							
							if(SH.c(sheet.attributeValue("minage")).length()>0 && Integer.parseInt(SH.c(sheet.attributeValue("minage")))>ageinmonths){
								continue;
							...
							t.activates.add(SH.c(sheet.attributeValue("id"))+";"+SH.c(sheet.attributeValue("value"))+";"+SH.c(sheet.attributeValue("reset"),"0"));
						}
					}
				}
				...
				while(pathways.hasNext()){
					...
					if(isPathwayApplicable(pathway,signs)){
						//Check if this pathway must be shown
						if(!bFirstChapter && mustHide(pathway, signs, treatments, sWebLanguage)){
							continue;
						}
						bFirstChapter=false;
						...
						if(bSignsToExclude){
							...
							while(childNodes.hasNext()){
								...
								if(iPaths.hasNext()){
									String title = (String)iPaths.next();
									out.println("<tr><td colspan='2' style='background-color: black;text-align: center'><font style='font-size:5vw;font-weight: bolder;color: white'>"+title.split("\\$")[0].split(">")[0]+"</font></td></tr>");
								}
								if(bHasTreatments){
									out.println("<tr><td colspan='2' style='border: 1px solid black;'><input type='hidden' name='reset' id='reset'/><table width='100%'><tr><td colspan='2'><font style='font-size:5vw;font-weight: bolder;color: grey'>"+getTran(request,"web","treatments",sWebLanguage)+"</font></td></tr>");
									iPaths = hPaths.keySet().iterator();
									while(iPaths.hasNext()){
										String title = (String)iPaths.next();
										if(!hasLaterNode(hPaths, title)){
											...
											if(lastpart.split("\\$").length>1 && treatments.get(lastpart.split("\\$")[1])!=null){
												...
												if(treatment.activates!=null && treatment.activates.size()>0){
													out.println("<tr><td/><td><table cellpadding='3'><tr>");
													for(int q=0;q<treatment.activates.size();q++){
														String sptToActivate=(String)treatment.activates.elementAt(q);
														out.println("<td style='background-color: #004369;'><input type='hidden' name='concept."+sptToActivate.split(";")[0]+"' id='concept."+sptToActivate.split(";")[0]+"'/><a href=\"javascript:document.getElementById('concept."+sptToActivate.split(";")[0]+"').value='"+sptToActivate.split(";")[1]+"';document.getElementById('reset').value='"+sptToActivate.split(";")[2]+"';transactionForm.submit();\"><font style='font-size:4vw;color: white;text-decoration: underline'>"+((Concept)concepts.get(sptToActivate.split(";")[0])).text+"</font></a></td>");
													}
													out.println("</tr></table></td></tr>");
												}
											}
