...
<%
    ...
    if(activeUser.getAccessRight("statistics.select")){        
        ...
        out.print("<tr>"+
                  ...
                  writeTblChildWithCode(activeUser.getAccessRight("statistics.lab.select"),"javascript:labMonthlyActivityReport()",getTran(request,"Web","statistics.labmonthlyactivityreport",sWebLanguage))+
                  writeTblChildWithCode(activeUser.getAccessRight("statistics.lab.select"),"javascript:labYearlyReport()",getTran(request,"Web","statistics.labyearlyreport",sWebLanguage))+
    ...
    if(activeUser.getAccessRight("statistics.select")){
        ...
        out.print(writeTblChildWithCodeNoButton(activeUser.getAccessRight("statistics.download.patients.select"),"javascript:downloadStats(\"patients.list\",\"openclinic\");",getTran(request,"Web","statistics.download.patients",sWebLanguage))+
                  ...
                  writeTblChildWithCodeNoButton(activeUser.getAccessRight("statistics.download.labsamples.select"),"javascript:downloadStats(\"lab.samples\",\"openclinic\");",getTran(request,"Web","statistics.download.labsamples",sWebLanguage))+
	...
	out.print(ScreenHelper.writeTblHeader(getTran(request,"Web","Declaration",sWebLanguage),sCONTEXTPATH));
	out.print(
	    writeTblChildWithCode(activeUser.getAccessRight("nationalreport.select"),"javascript:statsObrMedHub()",getTran(request,"Web","periodicsammary",sWebLanguage))
	     );
	out.print(ScreenHelper.writeTblFooter()+"<br>");
%>
</form>
...
<script>
function statsObrMedHub(){
	openPopup("/financial/medhub/PeriodicSummary.jsp&ts=<%=getTs()%>&PopupWidth=800&PopupHeight=600");
  }
  
function ExecuteObrMedHub(){
	openPopup("/financial/ExecuteDeclaration.jsp&ts=<%=getTs()%>&PopupWidth=800&PopupHeight=600");
  }
...
function downloadTechnicalStats(query,db){
  ...
  function labMonthlyActivityReport(){
	    var URL = "labos/manageLabMonthlyActivityReport.jsp&start="+document.getElementById('begintech').value+"&end="+document.getElementById('endtech').value+"&ts=<%=getTs()%>";
		openPopup(URL,800,600,"OpenClinic");
}    
  function labYearlyReport(){
	    var URL = "labos/manageLabYearlyReport.jsp&start="+document.getElementById('begintech').value+"&end="+document.getElementById('endtech').value+"&ts=<%=getTs()%>";
		openPopup(URL,1024,600,"OpenClinic");
}    
