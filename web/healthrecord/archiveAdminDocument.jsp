...
<%=checkPermission(out,"occup.arch.admindocuments","select",activeUser)%>
...
<form name="transactionForm" id="transactionForm" method="POST" action="<c:url value='/healthrecord/updateTransaction.do'/>?ts=<%=getTs()%>" focus='type'>
    ...
    <%=writeTableHeader("web.occup",sPREFIX+"TRANSACTION_TYPE_ARCHIVE_ADMINDOCUMENT",sWebLanguage,sCONTEXTPATH+"/main.do?Page=healthrecord/index.jsp&ts="+getTs())%>   
			        ...
			        <tr>
			            ...
			            <td class="admin2">
			                <select class="text" name="currentTransactionVO.items.<ItemVO[hashCode=<mxs:propertyAccessorI18N name="transaction.items" scope="page" compare="type=be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_DOC_CATEGORY" property="itemId"/>]>.value" id="category">
			                    <option/>
			                    <%=ScreenHelper.writeSelect(request,"arch.admindoc.category",tran.getItemValue(ScreenHelper.ITEM_PREFIX+"ITEM_TYPE_DOC_CATEGORY"),sWebLanguage)%>
			                </select>
			            </td>
			        </tr>
