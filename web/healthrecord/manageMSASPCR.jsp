...
<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    ...
    <table class="list" width="100%" cellspacing="1" cellpadding='0'>
        ...
        <tr>
        	...
        	<td class='admin'><%=getTran(request,"web","testorigin",sWebLanguage) %></td>
        	<td class='admin2'><%=SH.writeDefaultTextInput(session, tran, "ITEM_TYPE_TESTORIGIN", 50) %></td>
        </tr>
        <tr>
        ...
        <tr>
        	<td class='admin'><%=getTran(request,"web","prohylaxisarv",sWebLanguage) %></td>
        	<td class='admin2'><%=SH.writeDefaultSelect(request, tran, "ITEM_TYPE_PROHYLAXISARV", "yesno", sWebLanguage, "") %></td>
        	<td class='admin'><%=getTran(request,"web","prophylaxisarvschema",sWebLanguage) %></td>
        	<td class='admin2'><%=SH.writeDefaultTextArea(session, tran, "ITEM_TYPE_PROHYLAXISARVSCHEMA", 40, 1) %></td>
        </tr>
        <tr>
        	<td class='admin'><%=getTran(request,"web","startarvchild",sWebLanguage) %></td>
        	<td class='admin2'><%=SH.writeDefaultDateInput(session, tran, "ITEM_TYPE_STARTARVCHILD", sWebLanguage, sCONTEXTPATH) %></td>
        	<td class='admin'><%=getTran(request,"web","typeoffeeding",sWebLanguage) %></td>
        	<td class='admin2'><%=SH.writeDefaultSelect(request, tran, "ITEM_TYPE_TYPEOFFEEDING", "msas.pcr.feeding", sWebLanguage, "") %></td>
        </tr>
        <tr class='admin'>
        ...
        <tr>
        	<td colspan="4">
        		<table width='100%'>
        			<tr>
        				<td class='admin'></td>
        				<td class='admin'><%=getTran(request,"web","idlab",sWebLanguage) %></td>
        				...
        				<td class='admin'><%=getTran(request,"web","datetest",sWebLanguage) %></td>
        				<td class='admin'><%=getTran(request,"web","dateresult",sWebLanguage) %></td>
        				<td class='admin'><%=getTran(request,"web","result",sWebLanguage) %></td>
        			</tr>
        			<tr>
        				<td class='admin'><%=SH.writeDefaultSelect(request, tran, "ITEM_TYPE_PCRTYPE", "msas.pcr.type", sWebLanguage, "")%></td>
						<td class='admin2'><%=SH.writeDefaultTextInput(session, tran, "ITEM_TYPE_PCRID",10) %></td>
						<td class='admin2'><%=SH.writeDefaultDateInput(session, tran, "ITEM_TYPE_PCRSAMPLE", sWebLanguage,sCONTEXTPATH) %></td>
						<td class='admin2'><%=SH.writeDefaultDateInput(session, tran, "ITEM_TYPE_PCRSAMPLERECEIVED", sWebLanguage,sCONTEXTPATH) %></td>
						<td class='admin2'><%=SH.writeDefaultDateInput(session, tran, "ITEM_TYPE_PCRTEST", sWebLanguage,sCONTEXTPATH) %></td>
						<td class='admin2'><%=SH.writeDefaultDateInput(session, tran, "ITEM_TYPE_PCRRESULTDATE", sWebLanguage,sCONTEXTPATH) %></td>
						<td class='admin2'><%=SH.writeDefaultSelect(request, tran, "ITEM_TYPE_PCRRESULT", "posneg", sWebLanguage, "") %></td>
        			</tr>
        		</table>
        	</td>
        </tr>
        <tr class='admin'>
        	<td colspan="4"><%=getTran(request,"web","serology",sWebLanguage) %></td>
        </tr>
        <tr>
        	<td class='admin'><%=getTran(request,"web","dateserology",sWebLanguage) %></td>
        	<td class='admin2'><%=SH.writeDefaultDateInput(session, tran, "ITEM_TYPE_DATESEROLOGY", sWebLanguage, sCONTEXTPATH) %></td>
        	<td class='admin'><%=getTran(request,"pcr","childageinmonths",sWebLanguage) %></td>
        	<td class='admin2'><%=SH.writeDefaultNumericInput(session, tran, "ITEM_TYPE_CHILDAGE", 4, 0, 120, sWebLanguage) %></td>
        </tr>
        <tr>
        	<td class='admin'><%=getTran(request,"web","result",sWebLanguage) %></td>
						<td class='admin2'><%=SH.writeDefaultSelect(request, tran, "ITEM_TYPE_SEROLOGYRESULT", "posneg", sWebLanguage, "") %></td>
