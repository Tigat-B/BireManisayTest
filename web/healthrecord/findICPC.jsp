    ...
    <table class='menu' width='100%' cellspacing="0">
        ...
        <%
	        ...
	        %><tbody class="hand"><%
            		...
            		out.println("<tr><td colspan='3' style='font-size:12px;color: darkred'><br/>"+getTran(request,"web","keywordtoshort",sWebLanguage)+"<br/><br/></td></tr>");
            	}
  ...
  function addICD10(code,label){
    if (window.opener.document.getElementById('<%=sReturnField%>').innerHTML.search("ICD10Code"+code)==-1){
      ...
      if ((window.opener.document.getElementById('<%=sReturnField%>').type=="text")||(window.opener.document.getElementById('<%=sReturnField%>').type=="textarea")){
        ...
        openPopup('/_common/search/diagnoseInfo.jsp&EncounterUid=<%=sEncounterUid%>&AuthorUID=<%=sAuthorId%>&ts=<%=getTs()%>&showpatientencounters=<%=sShowPatientEncounters%>&Type=ICD10&Code='+code+'&Value='+encodeURI(document.getElementsByName("ICD10Comment"+code)[0].value)+'&Label='+encodeURI(label)+'&returnField=<%=sReturnField%>&returnField2=<%=sReturnField2%>&patientuid=<%=sPatientUid%>',800,600);
      }
    }
    else {
    	alertDialog("web","code.already.selected");
