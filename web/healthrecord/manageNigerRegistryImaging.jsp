...
<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    ...
    <table class="list" width="100%" cellspacing="1" cellpadding='0'>
		...
		<tr>
			<td class='admin'><%=getTran(request,"web","requestingdepartment",sWebLanguage) %></td>
			<td class='admin2' colspan='3'><%=SH.writeDefaultSelect(request, tran, "ITEM_TYPE_REQUESTINGSERVICE", "niger.imagingservice", sWebLanguage, "") %></td>
		</tr>
		<tr>
			<td class='admin'><%=getTran(request,"web","radiography",sWebLanguage) %></td>
			<td class='admin2'><%=SH.writeDefaultCheckBoxes(tran, request, "niger.radiography", "ITEM_TYPE_RADIO", sWebLanguage, false) %></td>
			<td class='admin'><%=getTran(request,"web","detail",sWebLanguage) %></td>
			<td class='admin2'><%=SH.writeDefaultTextArea(session, tran, "ITEM_TYPE_RADIODETAIL", 40, 1) %></td>
		</tr>
		<tr>
			<td class='admin'><%=getTran(request,"web","ultrasound",sWebLanguage) %></td>
			<td class='admin2'><%=SH.writeDefaultCheckBoxes(tran, request, "niger.ultrasound", "ITEM_TYPE_US", sWebLanguage, false) %></td>
			<td class='admin'><%=getTran(request,"web","detail",sWebLanguage) %></td>
			<td class='admin2'><%=SH.writeDefaultTextArea(session, tran, "ITEM_TYPE_USDETAIL", 40, 1) %></td>
		</tr>
		<tr>
			<td class='admin'><%=getTran(request,"web","scanner",sWebLanguage) %></td>
			<td class='admin2'><%=SH.writeDefaultCheckBoxes(tran, request, "niger.scanner", "ITEM_TYPE_SCANNER", sWebLanguage, false) %></td>
			<td class='admin'><%=getTran(request,"web","detail",sWebLanguage) %></td>
			<td class='admin2'><%=SH.writeDefaultTextArea(session, tran, "ITEM_TYPE_SCANNERDETAIL", 40, 1) %></td>
		</tr>
		<tr>
			<td class='admin'><%=getTran(request,"web","irm",sWebLanguage) %></td>
			<td class='admin2'><%=SH.writeDefaultCheckBoxes(tran, request, "niger.scanner", "ITEM_TYPE_IRM", sWebLanguage, false) %></td>
			<td class='admin'><%=getTran(request,"web","detail",sWebLanguage) %></td>
			<td class='admin2'><%=SH.writeDefaultTextArea(session, tran, "ITEM_TYPE_IRMDETAIL", 40, 1) %></td>
		</tr>
    </table>
