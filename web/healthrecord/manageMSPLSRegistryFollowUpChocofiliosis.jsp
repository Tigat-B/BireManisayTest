...
<!-- 
 ...
 -->
<%
	String accessright="mspls.chocofiliosis";
%>
...
<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    ...
    <table class="list" width="100%" cellspacing="1" cellpadding='0'>
        ...
        %>
        <tr>
        	<td class='admin'><%= getTran(request,"chocofiliosis","observations",sWebLanguage) %></td>
        	<td class='admin2'><%= SH.writeDefaultTextArea(session, tran, "ITEM_TYPE_OBSERVATIONS", 40, 2) %></td>
        	<td class='admin'><%= getTran(request,"chocofiliosis","diseasestage",sWebLanguage) %></td>
        	<td class='admin2'>
        		<%= SH.writeDefaultSelect(request, tran, "ITEM_TYPE_STAGE", "stage.chocofiliosis", sWebLanguage, "") %>
        	</td>
        </tr>
        <tr>
        	<td class='admin'><%= getTran(request,"chocofiliosis","signs",sWebLanguage) %></td>
        	<td class='admin2'><%= SH.writeDefaultCheckBoxes(tran, request, "signs.chocofiliosis", "ITEM_TYPE_SIGNS", sWebLanguage, true) %></td>
        	<td class='admin'><%= getTran(request,"chocofiliosis","TRC",sWebLanguage) %></td>
        	<td class='admin2'><%= SH.writeDefaultRadioButtons(tran, request, "trc.chocofiliosis", "ITEM_TYPE_TRC", sWebLanguage, true, "", "") %></td>
        </tr>
        <tr>
        	<td class='admin'><%= getTran(request,"chocofiliosis","chocolatequantity",sWebLanguage) %></td>
        	<td class='admin2'>
        		<%= SH.writeDefaultSelect(request, tran, "ITEM_TYPE_CHOCOLATETYPE", "chocolatetype.chocofiliosis", sWebLanguage, "") %>
        		<%= SH.writeDefaultNumericInput(session, tran, "ITEM_TYPE_CHOCOLATEWEIGHT", 5, sWebLanguage) %>g
        	</td>
        	<td class='admin'><%= getTran(request,"chocofiliosis","familyinfected",sWebLanguage) %></td>
        	<td class='admin2'><%= SH.writeDefaultTextArea(session, tran, "ITEM_TYPE_FAMILYINFECTED", 40, 2) %></td>
        </tr>
    </table>
