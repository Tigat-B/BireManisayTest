...
<%
	String accessright="mspls.adulthiv.therapeuticeducation.followup";
%>
...
<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    ...
    <table class="list" width="100%" cellspacing="1" cellpadding='0'>
        ...
        <tr>
        	<td class='admin'><%=getTran(request,"web","sessiontype",sWebLanguage) %></td>
        	<td class='admin2'><%=ScreenHelper.writeDefaultTextInput(session, (TransactionVO)transaction, "ITEM_TYPE_HIV_TE_SESSIONTYPE", 20) %></td>
        	<td class='admin'><%=getTran(request,"web","duration",sWebLanguage) %></td>
        	<td class='admin2'><%=ScreenHelper.writeDefaultNumericInput(session, (TransactionVO)transaction, "ITEM_TYPE_HIV_TE_SESSIONDURATION", 5,"") %>min</td>
        </tr>
        <tr>
        	<td class='admin'><%=getTran(request,"web","prioritytodiscuss",sWebLanguage)%></td>
        	<td class='admin2' colspan='3'><%=ScreenHelper.writeDefaultTextArea(session, (TransactionVO)transaction, "ITEM_TYPE_HIV_TE_PRIORITY", 80, 1)%></td>
        </tr>
        <tr>
        	<td class='admin'><%=getTran(request,"web","synthesis",sWebLanguage)%></td>
        	<td class='admin2' colspan='3'><%=ScreenHelper.writeDefaultTextArea(session, (TransactionVO)transaction, "ITEM_TYPE_HIV_TE_SYNTHESIS", 80, 1)%></td>
        </tr>
        <tr>
        	<td class='admin'><%=getTran(request,"web","nextappointment",sWebLanguage)%></td>
        	<td class='admin2'><%=ScreenHelper.writeDefaultDateInput(session, (TransactionVO)transaction, "ITEM_TYPE_HIV_TE_NEXTAPPOINTMENT", sWebLanguage, sCONTEXTPATH) %></td>
        	<td class='admin'><%=getTran(request,"web","prioritytodiscuss",sWebLanguage)%></td>
        	<td class='admin2'><%=ScreenHelper.writeDefaultTextArea(session, (TransactionVO)transaction, "ITEM_TYPE_HIV_TE_NEXTPRIORITY", 40, 1)%></td>
        </tr>
    </table>
