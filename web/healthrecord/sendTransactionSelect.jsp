...
<%
	...
	int destinationid=SH.ci("defaultGHBdestination",-1);
	String destinationname="";
	if(destinationid>-1){
		destinationname=GHBNetwork.getServerNameById(destinationid);
	}
	TransactionVO transaction = null;
	if(transactionId.length()>0 && serverId.length()>0){
		transaction = MedwanQuery.getInstance().loadTransaction(Integer.parseInt(serverId), Integer.parseInt(transactionId));
		if(transaction!=null && transaction.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_ORIGINALUID").split("\\.").length>1){
			int originserver=Integer.parseInt(transaction.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_ORIGINALUID").split("\\.")[0]);
			if(SH.ci("ghb_ref_serverid",-1)>-1 && SH.ci("ghb_ref_serverid",-1)!=originserver){
				destinationid=originserver;
				destinationname=GHBNetwork.getServerNameById(destinationid);
			}
		}
	}
