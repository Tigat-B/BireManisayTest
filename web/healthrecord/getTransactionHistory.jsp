		...
		try{
			...
			if(tran!=null){
				...
				for(int n=tran.getVersion();n>0 && n>tran.getVersion()-depth;n--){
					TransactionVO historytran=MedwanQuery.getInstance().loadTransaction(tran.getServerId()+"", tran.getTransactionId()+"", n+"", SH.getServerId()+"");
					if(historytran!=null){
						if(historytran.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_REFERRAL_USER").length()>0){
							sUserName="<font style='color: red;font-weight: bolder'>"+historytran.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_REFERRAL_USER")+
									" ["+historytran.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_REFERRAL_SOURCESITE")+"]</font>";
						}
						else{
			...
			if(request.getParameter("transactionversion")!=null){
				...
				tran=MedwanQuery.getInstance().loadTransaction(tran.getServerId()+"", tran.getTransactionId()+"", request.getParameter("transactionversion"), SH.getServerId()+"");
                //load all missing items for this type of transaction
                Vector items = MedwanQuery.getInstance().loadTransactionItems(tran.getTransactionType(),null,false);
               	for(int n=0;n<items.size();n++){
               		ItemVO item =(ItemVO)items.elementAt(n);
               		if(tran.getItem(item.getType())==null){
