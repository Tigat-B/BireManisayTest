...
<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    ...
    <table class="list" width="100%" cellspacing="1" cellpadding='0'>
        ...
        <% if(SH.cs("countrycode","bi").equalsIgnoreCase("ml")){ %>
        <tr>
            <td class="admin"><%=getTran(request,"web", "fpcentertype", sWebLanguage)%></td>
            <td class="admin2">
            	<%=ScreenHelper.writeDefaultRadioButtons((TransactionVO)transaction, request, "fp.centertype", "!ITEM_TYPE_FP_CENTERTYPE", sWebLanguage, true, "", "") %>
            </td>
            <td class="admin"><%=getTran(request,"web", "postpartum", sWebLanguage)%></td>
            <td class="admin2">
            	<%=ScreenHelper.writeDefaultRadioButtons((TransactionVO)transaction, request, "yesno", "ITEM_TYPE_FP_POSTPARTUM", sWebLanguage, true, "", "") %>
            </td>
        </tr>
        <% } %>
        <tr class='admin'>
		...
		<tr>
        	...
        	<td class='admin2' style='padding: 0px' colspan='1'>
        		<%=ScreenHelper.writeDefaultSelect(request, (TransactionVO)transaction, "selectedmethod", SH.cs("fpSelectedMethod","fp.selectedmethod.mspls"), sWebLanguage, "") %>
        	</td>
		...
		<tr>
        	...
        	<td class='admin2' style='padding: 0px' colspan='3'>
        		<%=ScreenHelper.writeDefaultCheckBoxes((TransactionVO)transaction, request, "fp.otherinformation", "otherinformation", sWebLanguage, true, "onchange='if(this.id==\"otherinformation.1\" && this.checked){document.getElementById(\"otherinformation.2\").checked=false;document.getElementById(\"otherinformation\").value=document.getElementById(\"otherinformation\").value.replace(\"2;\",\"\")};if(this.id==\"otherinformation.2\" && this.checked){document.getElementById(\"otherinformation.1\").checked=false;document.getElementById(\"otherinformation\").value=document.getElementById(\"otherinformation\").value.replace(\"1;\",\"\")}'") %>
        	</td>
		</tr>
		<tr>
		...
		<tr>
        	<td class='admin' colspan='1'><%=getTran(request,"web","pfcounseling",sWebLanguage) %></td>
        	<td class='admin2' style='padding: 0px' colspan='1'>
        		<%=ScreenHelper.writeDefaultRadioButtons((TransactionVO)transaction, request, "yesno", "ITEM_TYPE_PF_COUNSELING", sWebLanguage, false, "", "") %>
        	</td>
        	<td class='admin' colspan='1'><%=getTran(request,"web","pfinformationsources",sWebLanguage) %></td>
        	<td class='admin2' style='padding: 0px' colspan='1'>
        		<%=ScreenHelper.writeDefaultCheckBoxes((TransactionVO)transaction, request, "fp.informationsources", "ITEM_TYPE_PF_INFORMATIONSOURCES", sWebLanguage, true, "") %>
        	</td>
		</tr>
    </table>
