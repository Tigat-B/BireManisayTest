...
<%!
	...
	StringBuffer html = new StringBuffer();
        ...
        for(int i=0; i<objects.size(); i++){
            ...
            if(sProductUid!=null && !sProductUid.equals(sPreviousProductUid)){
			...
			try{
				...
				SH.syslog("Error in prescription rule");
				e.printStackTrace();
			}
...
<%
    ...
    if(sAction.equals("save") && sEditPrescrUid.length() > 0){
		...
		out.println("<script>if(window.opener.loadActivePrescriptions){window.opener.loadActivePrescriptions();}</script>");
		out.flush();
...
<form name="transactionForm" id="transactionForm" method="post" action="<c:url value='/'/>/popup.jsp?Page=medical/managePrescriptionsPopup.jsp&PopupHeight=400&PopupWidth=900" <%=sOnKeyDown%> onClick='setSaveButton(event);clearMessage();' onKeyUp='setSaveButton(event);'>
    <input type="hidden" name="noParentReload" value="<%=SH.p(request,"noParentReload")%>">
...
<tr>
    ...
    <td class="admin2">
        <input class="text" type="text" name="EditAuthorization" id="EditAuthorization" size="<%=sTextWidth%>" value="<%=sSelectedAuthorization%>">
    </td>
</tr>
